...
use lemurclaw_skills::EnvironmentSkillMetadata;
...
impl ExecutorSkillProvider {
    fn list_from_discovery(
        &self,
        snapshot: &lemurclaw_exec_server::ExecutorCapabilityDiscoverySnapshot,
    ) -> SkillCatalog {
        let mut catalog = SkillCatalog::default();
...
async fn read_bounded_text(
    file_system: &dyn lemurclaw_exec_server::ExecutorFileSystem,
    path: &PathUri,
    resource: &str,
