...
use lemurclaw_extension_api::ThreadIdleInput;
...
use lemurclaw_extension_api::ToolLifecycleFuture;
use lemurclaw_extension_api::TurnAbortInput;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                lemurclaw_protocol::config_types::ModeKind::Plan
            ) {
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn lemurclaw_extension_api::ToolExecutor<lemurclaw_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
