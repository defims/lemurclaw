...
use lemurclaw_extension_api::JsonToolOutput;
use lemurclaw_extension_api::ToolCall;
use lemurclaw_extension_api::ToolExecutor;
use lemurclaw_extension_api::ToolName;
use lemurclaw_extension_api::ToolOutput;
use lemurclaw_extension_api::ToolSpec;
...
use lemurclaw_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<lemurclaw_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> lemurclaw_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => lemurclaw_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => lemurclaw_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: lemurclaw_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            lemurclaw_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            lemurclaw_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<lemurclaw_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &lemurclaw_state::StateRuntime,
    thread_id: ThreadId,
    goal: &lemurclaw_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: lemurclaw_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: lemurclaw_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> lemurclaw_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => lemurclaw_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => lemurclaw_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => lemurclaw_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => lemurclaw_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => lemurclaw_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => lemurclaw_state::ThreadGoalStatus::Complete,
    }
}
