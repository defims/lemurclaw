...
use lemurclaw_extension_api::FunctionCallError;
use lemurclaw_extension_api::NoopTurnItemEmitter;
use lemurclaw_extension_api::ThreadResumeInput;
use lemurclaw_extension_api::ThreadStartInput;
use lemurclaw_extension_api::ThreadStopInput;
use lemurclaw_extension_api::ToolCall;
use lemurclaw_extension_api::ToolCallOutcome;
use lemurclaw_extension_api::ToolCallSource;
use lemurclaw_extension_api::ToolExecutor;
use lemurclaw_extension_api::ToolFinishInput;
use lemurclaw_extension_api::ToolPayload;
use lemurclaw_extension_api::TurnErrorInput;
use lemurclaw_extension_api::TurnStartInput;
use lemurclaw_extension_api::TurnStopInput;
use lemurclaw_goal_extension::GoalObjectiveUpdate;
use lemurclaw_goal_extension::GoalRuntimeHandle;
use lemurclaw_goal_extension::GoalService;
use lemurclaw_goal_extension::GoalSetRequest;
use lemurclaw_goal_extension::GoalTokenBudgetUpdate;
use lemurclaw_goal_extension::install_with_backend;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(lemurclaw_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(lemurclaw_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(lemurclaw_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(lemurclaw_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            lemurclaw_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<lemurclaw_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: lemurclaw_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = lemurclaw_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: lemurclaw_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        codex_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: lemurclaw_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<lemurclaw_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    lemurclaw_state::StateRuntime::init(
        lemurclaw_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &lemurclaw_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = lemurclaw_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: lemurclaw_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        lemurclaw_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        lemurclaw_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        lemurclaw_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        lemurclaw_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        lemurclaw_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        lemurclaw_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
