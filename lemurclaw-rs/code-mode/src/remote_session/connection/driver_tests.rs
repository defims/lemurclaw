...
use lemurclaw_code_mode_protocol::CellId;
use lemurclaw_code_mode_protocol::CodeModeNestedToolCall;
use lemurclaw_code_mode_protocol::CodeModeSessionDelegate;
use lemurclaw_code_mode_protocol::ExecuteRequest;
use lemurclaw_code_mode_protocol::NotificationFuture;
use lemurclaw_code_mode_protocol::ToolInvocationFuture;
use lemurclaw_code_mode_protocol::WaitRequest;
use lemurclaw_code_mode_protocol::host::ClientToHost;
use lemurclaw_code_mode_protocol::host::DelegateRequest;
use lemurclaw_code_mode_protocol::host::DelegateRequestId;
use lemurclaw_code_mode_protocol::host::EncodedFrame;
use lemurclaw_code_mode_protocol::host::HostResponse;
use lemurclaw_code_mode_protocol::host::HostToClient;
use lemurclaw_code_mode_protocol::host::MAX_PENDING_DELEGATE_CALLS;
use lemurclaw_code_mode_protocol::host::RequestId;
use lemurclaw_code_mode_protocol::host::SessionId;
use lemurclaw_code_mode_protocol::host::WireNestedToolCall;
use lemurclaw_code_mode_protocol::host::WireResult;
use lemurclaw_code_mode_protocol::host::WireRuntimeResponse;
use lemurclaw_code_mode_protocol::host::WireWaitOutcome;
...
struct DriverHarness {
    ...
    outgoing_rx: mpsc::Receiver<lemurclaw_code_mode_protocol::host::EncodedFrame>,
...
impl DriverHarness {
    ...
    async fn start_cell(
        ...
        request_id: i64,
        cell_id: &str,
    ) -> lemurclaw_code_mode_protocol::StartedCell {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            ...
            .send(DriverEvent::HostMessage(HostToClient::DelegateRequest {
                ...
                request: DelegateRequest::InvokeTool {
                    invocation: WireNestedToolCall {
                        ...
                        tool_kind: lemurclaw_code_mode_protocol::CodeModeToolKind::Function.into(),
                        input: None,
                    },
                },
            }))
...
async fn terminate_closes_cell_without_waiting_for_delegate_cleanup() {
    ...
    assert_eq!(
        ...
        Ok(lemurclaw_code_mode_protocol::WaitOutcome::LiveCell(
            lemurclaw_code_mode_protocol::RuntimeResponse::Terminated {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))
...
async fn remote_wait_accepts_durations_longer_than_five_minutes() {
    ...
    assert_eq!(
        ...
        Ok(lemurclaw_code_mode_protocol::WaitOutcome::LiveCell(
            lemurclaw_code_mode_protocol::RuntimeResponse::Yielded {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))
