...
use lemurclaw_goal_extension::GoalServiceError;
...
use lemurclaw_protocol::protocol::ThreadSettingsSnapshot;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn thread_goal_set_inner(
        ...
        let persist_result = match self.thread_manager.get_thread(thread_id).await {
            Ok(thread) => match thread.rollout_path() {
                Some(path) if lemurclaw_rollout::existing_rollout_path(&path).await.is_none() => {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            lemurclaw_rollout::find_thread_path_by_id_str(
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => lemurclaw_rollout::find_thread_path_by_id_str(
                &self.config.codex_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
...
pub(super) fn api_thread_goal_from_state(goal: lemurclaw_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: lemurclaw_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
