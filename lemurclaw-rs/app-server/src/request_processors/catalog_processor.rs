use super::*;
use lemurclaw_core::config::permission_profile_catalog;
...
) -> Vec<lemurclaw_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            lemurclaw_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    lemurclaw_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| lemurclaw_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[lemurclaw_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[lemurclaw_core::skills::SkillError],
) -> Vec<lemurclaw_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| lemurclaw_app_server_protocol::SkillErrorInfo {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                lemurclaw_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![lemurclaw_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let skills_input = lemurclaw_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(lemurclaw_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![lemurclaw_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let plugin_hooks = if plugins_enabled {
                ...
                lemurclaw_core_plugins::PluginHookLoadOutcome {
                ...
                lemurclaw_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = lemurclaw_hooks::list_hooks(lemurclaw_hooks::HooksConfig {
