...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        lemurclaw_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    use lemurclaw_state::ThreadMetadataBuilder;
    ...
    fn dynamic_tool(
        ...
        match namespace {
            Some(namespace) => {
                DynamicToolSpec::Namespace(lemurclaw_app_server_protocol::DynamicToolNamespaceSpec {
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        let tools = vec![DynamicToolSpec::Namespace(
            lemurclaw_app_server_protocol::DynamicToolNamespaceSpec {
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(lemurclaw_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn requested_permissions_trust_project_uses_permission_profile_intent() {
        ...
        let full_access_profile = lemurclaw_protocol::models::PermissionProfile::Disabled;
        let workspace_write_profile = lemurclaw_protocol::models::PermissionProfile::workspace_write();
        let read_only_profile = lemurclaw_protocol::models::PermissionProfile::read_only();
        let split_write_profile =
            lemurclaw_protocol::models::PermissionProfile::from_runtime_permissions(
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: lemurclaw_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: lemurclaw_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: lemurclaw_protocol::models::PermissionProfile::Disabled,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            lemurclaw_protocol::protocol::SessionSource::default(),
        );
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(lemurclaw_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(lemurclaw_protocol::protocol::ThreadSource::Subagent),
