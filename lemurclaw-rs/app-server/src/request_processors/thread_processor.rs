...
use lemurclaw_app_server_protocol::SelectedCapabilityRoot;
use lemurclaw_app_server_protocol::ThreadSection;
use lemurclaw_app_server_protocol::ThreadSectionListParams;
use lemurclaw_app_server_protocol::ThreadSectionListResponse;
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: lemurclaw_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                lemurclaw_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = lemurclaw_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(lemurclaw_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<lemurclaw_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<lemurclaw_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                lemurclaw_core::config::set_project_trust_level(
        ...
        if let Ok(Some(err)) =
            lemurclaw_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(lemurclaw_app_server_protocol::ThreadStartSource::Startup)
                {
                    lemurclaw_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    lemurclaw_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<lemurclaw_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<lemurclaw_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(lemurclaw_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(lemurclaw_app_server_protocol::ApprovalsReviewer::to_core),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = lemurclaw_core::util::normalize_thread_name(&name) else {
    ...
    ) -> Result<lemurclaw_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(lemurclaw_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    lemurclaw_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    lemurclaw_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => lemurclaw_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(lemurclaw_thread_store::PrepareForkParams {
...
fn deserialize_stored_thread_item(
    item: lemurclaw_thread_store::StoredThreadItem,
...
) -> (Thread, Option<lemurclaw_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<lemurclaw_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(lemurclaw_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match lemurclaw_core::parse_turn_item(item) {
                Some(lemurclaw_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            lemurclaw_protocol::config_types::SandboxMode::WorkspaceWrite
                | lemurclaw_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &lemurclaw_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        lemurclaw_protocol::models::PermissionProfile::Disabled
        | lemurclaw_protocol::models::PermissionProfile::External { .. } => true,
        lemurclaw_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<lemurclaw_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
