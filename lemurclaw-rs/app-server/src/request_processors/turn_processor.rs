use super::*;
use lemurclaw_agent_extension::AgentInvocation;
use lemurclaw_agent_extension::AgentRun;
use lemurclaw_agent_extension::AgentRunner;
...
use lemurclaw_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use lemurclaw_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
use lemurclaw_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<lemurclaw_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = lemurclaw_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input {
            ...
            lemurclaw_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<lemurclaw_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(lemurclaw_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(lemurclaw_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(lemurclaw_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != lemurclaw_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            lemurclaw_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            lemurclaw_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            lemurclaw_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
