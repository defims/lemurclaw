...
use lemurclaw_connectors::connector_runtime_cache_path;
use lemurclaw_feedback::CODEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME;
use lemurclaw_feedback::CODEX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME;
#[cfg(target_os = "windows")]
use lemurclaw_feedback::WINDOWS_SANDBOX_LOG_ATTACHMENT_FILENAME;
...
impl FeedbackRequestProcessor {
    ...
    async fn upload_feedback_response(
        ...
        if let Some(chatgpt_user_id) = auth
            .as_ref()
            .and_then(lemurclaw_login::CodexAuth::get_chatgpt_user_id)
        {
            tracing::info!(target: "feedback_tags", chatgpt_user_id);
        }
        if let Some(account_id) = auth
            .as_ref()
            .and_then(lemurclaw_login::CodexAuth::get_account_id)
        {
...
fn windows_sandbox_log_attachment(codex_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path = lemurclaw_windows_sandbox::current_log_file_path_for_codex_home(codex_home);
    sandbox_log_path
        .is_file()
...
mod tests {
    ...
    fn feedback_rollout(
        ...
        let mut lines = vec![RolloutLine {
            ...
            item: RolloutItem::SessionMeta(SessionMetaLine {
                meta: lemurclaw_protocol::protocol::SessionMeta {
        ...
        lines.extend(turns.iter().map(|(turn_id, model, effort)| {
            RolloutLine {
                ...
                item: RolloutItem::TurnContext(TurnContextItem {
                    ...
                    current_date: None,
                    timezone: None,
                    approval_policy: lemurclaw_protocol::protocol::AskForApproval::Never,
                    approvals_reviewer: None,
                    sandbox_policy: lemurclaw_protocol::protocol::SandboxPolicy::new_read_only_policy(),
    ...
    fn tool_cache_feedback_attachments_include_directory_cache_without_account_id() {
        let codex_home = tempfile::tempdir().expect("create tempdir");
        let auth = CodexAuth::Headers(lemurclaw_login::AuthHeaders::new(
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let codex_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = lemurclaw_windows_sandbox::sandbox_dir(codex_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            lemurclaw_windows_sandbox::current_log_file_path_for_codex_home(codex_home.path());
