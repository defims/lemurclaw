...
use lemurclaw_app_server_protocol::BrowserUseRequirements;
use lemurclaw_app_server_protocol::ClientResponsePayload;
use lemurclaw_app_server_protocol::ComputerUseRequirements;
use lemurclaw_app_server_protocol::ConfigBatchWriteParams;
use lemurclaw_app_server_protocol::ConfigReadParams;
use lemurclaw_app_server_protocol::ConfigReadResponse;
use lemurclaw_app_server_protocol::ConfigRequirements;
use lemurclaw_app_server_protocol::ConfigRequirementsReadResponse;
use lemurclaw_app_server_protocol::ConfigValueWriteParams;
use lemurclaw_app_server_protocol::ConfigWriteErrorCode;
use lemurclaw_app_server_protocol::ConfigWriteResponse;
use lemurclaw_app_server_protocol::ConfiguredHookHandler;
use lemurclaw_app_server_protocol::ConfiguredHookMatcherGroup;
use lemurclaw_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use lemurclaw_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use lemurclaw_app_server_protocol::FeedbackRequirements;
use lemurclaw_app_server_protocol::JSONRPCErrorError;
use lemurclaw_app_server_protocol::ManagedHooksRequirements;
use lemurclaw_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use lemurclaw_app_server_protocol::ModelsRequirements;
use lemurclaw_app_server_protocol::NetworkDomainPermission;
use lemurclaw_app_server_protocol::NetworkRequirements;
use lemurclaw_app_server_protocol::NetworkUnixSocketPermission;
...
use lemurclaw_config::HookEventsToml;
use lemurclaw_config::HookHandlerConfig as CoreHookHandlerConfig;
use lemurclaw_config::ManagedHooksRequirementsToml;
use lemurclaw_config::MatcherGroup as CoreMatcherGroup;
use lemurclaw_config::ResidencyRequirement as CoreResidencyRequirement;
use lemurclaw_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use lemurclaw_core::ThreadManager;
use lemurclaw_features::canonical_feature_for_key;
use lemurclaw_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<lemurclaw_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = lemurclaw_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(lemurclaw_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(lemurclaw_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            lemurclaw_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            lemurclaw_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: lemurclaw_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: lemurclaw_config::BrowserUseRequirementsToml,
...
) -> lemurclaw_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => lemurclaw_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: lemurclaw_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(lemurclaw_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(lemurclaw_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(lemurclaw_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: lemurclaw_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        lemurclaw_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        lemurclaw_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: lemurclaw_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        lemurclaw_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        lemurclaw_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use lemurclaw_app_server_protocol::FeedbackRequirements;
    use lemurclaw_app_server_protocol::WindowsSandboxSetupMode;
    use lemurclaw_config::ComputerUseRequirementsToml;
    use lemurclaw_config::ConfigRequirementsToml;
    use lemurclaw_config::ModelsRequirementsToml;
    use lemurclaw_config::NewThreadModelDefaultsToml;
    use lemurclaw_config::WindowsRequirementsToml;
