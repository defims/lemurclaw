...
use lemurclaw_code_mode::WebSocketCodeModeSessionProvider;
...
use lemurclaw_analytics::AppServerRpcTransport;
...
use lemurclaw_app_server_protocol::TextPosition as AppTextPosition;
use lemurclaw_app_server_protocol::TextRange as AppTextRange;
use lemurclaw_config::ConfigLayerSource;
use lemurclaw_config::ConfigLoadError;
use lemurclaw_config::TextRange as CoreTextRange;
use lemurclaw_core::ExecPolicyError;
...
use lemurclaw_rollout::state_db as rollout_state_db;
...
pub async fn run_main_with_transport_options(
    ...
    let otel = lemurclaw_core::otel_init::build_provider(
        &config,
    ...
    lemurclaw_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    lemurclaw_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !lemurclaw_state::is_sqlite_corruption_error(&err)
        ...
        let backups = lemurclaw_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
