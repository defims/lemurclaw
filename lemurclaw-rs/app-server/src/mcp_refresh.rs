...
mod tests {
    ...
    use lemurclaw_config::ThreadConfigContext;
    use lemurclaw_config::ThreadConfigLoadError;
    use lemurclaw_config::ThreadConfigLoadErrorCode;
    ...
    use lemurclaw_extension_api::NoopExtensionEventSink;
    ...
    async fn strict_refresh_reports_thread_planning_failures() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(lemurclaw_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_config_preserves_thread_mcp_overrides() -> anyhow::Result<()> {
        ...
        let thread_config = initial_config_manager
            ...
            .await?;
        let thread = thread_manager
            .start_thread(lemurclaw_core::StartThreadOptions::new(thread_config))
            .await?
            .thread;
        std::fs::write(
            temp_dir.path().join(lemurclaw_config::CONFIG_TOML_FILE),
...
"#,
        ...
        actual.remove(lemurclaw_mcp::CODEX_APPS_MCP_SERVER_NAME);
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let executor_skill_provider: Arc<dyn lemurclaw_skills_extension::SkillProvider> = Arc::new(
            lemurclaw_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                lemurclaw_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: lemurclaw_analytics::AnalyticsEventsClient::disabled(),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(lemurclaw_goal_extension::GoalService::new()),
                ...
                lemurclaw_core::local_agent_graph_store_from_state_db(Some(&state_db)),
        ...
        thread_manager
            .start_thread(lemurclaw_core::StartThreadOptions::new(good_config))
            .await?;
        thread_manager
            .start_thread(lemurclaw_core::StartThreadOptions::new(bad_config))
            .await?;
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> lemurclaw_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
