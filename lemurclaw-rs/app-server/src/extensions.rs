...
use lemurclaw_extension_api::AgentSpawnFuture;
use lemurclaw_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn lemurclaw_skills_extension::SkillProvider>,
    ...
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodexErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        lemurclaw_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            lemurclaw_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(lemurclaw_features::Feature::Goals),
        );
    }
    lemurclaw_git_attribution::install(
        &mut builder,
        auth_manager.clone(),
        git_attribution_base_url,
        http_client_factory,
    );
    lemurclaw_guardian::install(&mut builder, guardian_agent_spawner);
    lemurclaw_memories_extension::install(&mut builder, lemurclaw_otel::global());
    lemurclaw_mcp_extension::install(&mut builder);
    lemurclaw_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    lemurclaw_web_search_extension::install(&mut builder, auth_manager.clone());
    lemurclaw_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.codex_home.clone())
    });
    let skill_providers = lemurclaw_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            lemurclaw_skills_extension::OrchestratorSkillProvider::new(),
        ))
        .with_host_provider(Arc::new(lemurclaw_skills_extension::HostSkillProvider::new()));
    lemurclaw_skills_extension::install_with_providers_and_metrics(
        &mut builder,
        skill_providers,
        lemurclaw_otel::global(),
        |config: &Config| lemurclaw_skills_extension::SkillsExtensionConfig {
            include_instructions: config.include_skill_instructions,
            bundled_skills_enabled: config.bundled_skills_enabled(),
            orchestrator_skills_enabled: config.orchestrator_skills_enabled,
            shadow_selection_enabled: config
                .features
                .enabled(lemurclaw_features::Feature::SkillSearch),
        },
    );
...
#[cfg(test)]
mod tests {
    use lemurclaw_protocol::protocol::ThreadGoal as CoreThreadGoal;
