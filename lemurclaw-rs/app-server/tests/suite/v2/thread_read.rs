...
use lemurclaw_app_server::in_process;
...
use lemurclaw_core::ARCHIVED_SESSIONS_SUBDIR;
...
use lemurclaw_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use lemurclaw_thread_store::AppendThreadItemsParams;
use lemurclaw_thread_store::CreateThreadParams;
use lemurclaw_thread_store::InMemoryThreadStore;
use lemurclaw_thread_store::LocalThreadStore;
use lemurclaw_thread_store::LocalThreadStoreConfig;
use lemurclaw_thread_store::ThreadMetadataPatch;
use lemurclaw_thread_store::ThreadPersistenceMetadata;
use lemurclaw_thread_store::ThreadStore;
use lemurclaw_thread_store::UpdateThreadMetadataParams;
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let codex_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(codex_home.path())?;
    let thread_id = lemurclaw_protocol::ThreadId::default();
    let sqlite = lemurclaw_state::SqliteConfig::new_for_testing(codex_home.path().abs());
    let state_db =
        lemurclaw_state::StateRuntime::init(sqlite.clone(), "mock_provider".to_string()).await?;
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: lemurclaw_protocol::protocol::ThreadHistoryMode::Paginated,
            history_base: None,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            lemurclaw_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id = lemurclaw_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(lemurclaw_config::NoopThreadConfigLoader),
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = lemurclaw_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id = lemurclaw_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    MockResponsesConfig::new("http://127.0.0.1:1")
        .with_root_config(&format!(
            r#"experimental_thread_store = {{ type = "in_memory", id = "{store_id}" }}"#
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(lemurclaw_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<lemurclaw_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
async fn read_turns_page(
    mcp: &mut TestAppServer,
    thread_id: lemurclaw_protocol::ThreadId,
...
fn turn_user_texts(turns: &[lemurclaw_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[lemurclaw_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
