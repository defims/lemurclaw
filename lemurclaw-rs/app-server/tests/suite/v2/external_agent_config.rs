...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(lemurclaw_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let state_db = lemurclaw_state::StateRuntime::init(
        lemurclaw_state::SqliteConfig::new_for_testing(sqlite_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let memory_diff = lemurclaw_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    lemurclaw_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                lemurclaw_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                lemurclaw_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![lemurclaw_app_server_protocol::PluginsMigration {
