...
use lemurclaw_app_server::in_process::InProcessClientHandle;
use lemurclaw_app_server::in_process::InProcessServerEvent;
...
use lemurclaw_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
fn assert_no_local_persistence_artifacts(codex_home: &Path) -> Result<()> {
    ...
    assert!(
        !lemurclaw_state::SqliteConfig::new_for_testing(codex_home.abs())
            .state_db_path()
            .exists(),
