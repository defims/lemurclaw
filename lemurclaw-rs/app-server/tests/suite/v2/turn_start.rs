...
use lemurclaw_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use lemurclaw_app_server::INVALID_PARAMS_ERROR_CODE;
use lemurclaw_app_server_protocol::AdditionalContextEntry;
use lemurclaw_app_server_protocol::AdditionalContextKind;
use lemurclaw_app_server_protocol::ByteRange;
...
use lemurclaw_app_server_protocol::FileChangePatchUpdatedNotification;
...
use lemurclaw_app_server_protocol::JSONRPCError;
...
use lemurclaw_app_server_protocol::TextElement;
...
use lemurclaw_core::test_support::all_model_presets;
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: lemurclaw_app_server_protocol::ServerNotification = notification.try_into()?;
    let lemurclaw_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: lemurclaw_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    assert_eq!(
        started.turn.status,
        lemurclaw_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
    ...
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            ...
            params: TurnStartParams {
                ...
                approval_policy: Some(lemurclaw_app_server_protocol::AskForApproval::Never),
                sandbox_policy: Some(lemurclaw_app_server_protocol::SandboxPolicy::DangerFullAccess),
                model: Some("mock-model".to_string()),
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let first_writable_root =
        lemurclaw_utils_absolute_path::AbsolutePathBuf::try_from(first_cwd.clone())?;
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            request_id,
            params: TurnStartParams {
                ...
                sandbox_policy: Some(lemurclaw_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = lemurclaw_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = lemurclaw_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![lemurclaw_app_server_protocol::FileUpdateChange {
...
async fn direct_input_to_multi_agent_v2_subagent_is_rejected() -> Result<()> {
    ...
    let listed: lemurclaw_app_server_protocol::ThreadListResponse = mcp
        .request(|request_id| ClientRequest::ThreadList {
            request_id,
            params: lemurclaw_app_server_protocol::ThreadListParams {
                cursor: None,
                limit: Some(10),
                sort_key: None,
                sort_direction: None,
                ...
                source_kinds: Some(vec![
                    lemurclaw_app_server_protocol::ThreadSourceKind::SubAgentThreadSpawn,
                ]),
                archived: None,
                section_id: None,
                cwd: None,
    ...
    assert!(matches!(
        &listed_child.source,
        lemurclaw_app_server_protocol::SessionSource::SubAgent(
            lemurclaw_protocol::protocol::SubAgentSource::ThreadSpawn {
                agent_path: Some(_),
                ..
            }
        )
