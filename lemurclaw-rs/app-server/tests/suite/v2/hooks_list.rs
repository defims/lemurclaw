...
use lemurclaw_app_server_protocol::HookHandlerType;
use lemurclaw_app_server_protocol::HookMetadata;
use lemurclaw_app_server_protocol::HookSource;
...
fn command_hook_hash(
    ...
    let Ok(value) = lemurclaw_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    lemurclaw_config::version_for_toml(&value)
}
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    let write_id = mcp
        ...
        .await?;
    let _: lemurclaw_app_server_protocol::ConfigWriteResponse =
