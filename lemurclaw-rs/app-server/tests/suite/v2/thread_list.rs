...
use lemurclaw_git_utils::GitSha;
...
use lemurclaw_protocol::protocol::SessionSource as CoreSessionSource;
...
use lemurclaw_state::DirectionalThreadSpawnEdgeStatus;
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let mut mcp = init_mcp(codex_home.path()).await?;
    let request_id = mcp
        .send_thread_list_request(lemurclaw_app_server_protocol::ThreadListParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = lemurclaw_rollout::RolloutConfig {
        codex_home: codex_home.path().to_path_buf(),
        sqlite: lemurclaw_state::SqliteConfig::new_for_testing(codex_home.path().abs()),
        cwd: codex_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = lemurclaw_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        lemurclaw_core::ThreadSortKey::CreatedAt,
        lemurclaw_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(codex_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(lemurclaw_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
...
async fn thread_list_sort_recency_at_uses_state_db_order_with_provider_filter() -> Result<()> {
    ...
    lemurclaw_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        lemurclaw_core::ThreadSortKey::CreatedAt,
        lemurclaw_core::SortDirection::Desc,
        lemurclaw_core::INTERACTIVE_SESSION_SOURCES.as_slice(),
        /*model_providers*/ None,
        /*cwd_filters*/ None,
        "mock_provider",
        /*search_term*/ None,
