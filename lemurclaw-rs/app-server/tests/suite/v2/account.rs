...
use lemurclaw_app_server_protocol::ChatgptAuthTokensRefreshReason;
use lemurclaw_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use lemurclaw_login::CLIENT_ID_OVERRIDE_ENV_VAR;
use lemurclaw_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use lemurclaw_login::auth::BedrockApiKeyAuth;
use lemurclaw_login::load_auth_dot_json;
...
const LOGIN_ISSUER_ENV_VAR: &str = "LEMURCLAW_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "LEMURCLAW_APP_SERVER_DEV_OPEN_APP_URL";
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(lemurclaw_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: lemurclaw_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(lemurclaw_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![lemurclaw_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: lemurclaw_app_server_protocol::TurnStartResponse =
