...
`lemurclaw app-server` is the interface lemurclaw uses to power rich interfaces such as the [lemurclaw VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `lemurclaw app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
The unix socket transport is intended for local app-server control-plane clients. `lemurclaw app-server proxy`
...
Currently, you can dump a TypeScript version of the schema using `lemurclaw app-server generate-ts`, or a JSON Schema bundle via `lemurclaw app-server generate-json-schema`. Each output is specific to the version of lemurclaw you used to run the command, so the generated artifacts are guaranteed to match that version.

```
lemurclaw app-server generate-ts --out DIR
lemurclaw app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and lemurclaw:

- **Thread**: A conversation between a user and the lemurclaw agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's lemurclaw home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
Applications building on top of `lemurclaw app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new lemurclaw integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-lemurclaw
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "codex_vscode",
      "title": "lemurclaw VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and `version` to override configured realtime selection for this session only, pass `includeStartupContext: false` to omit lemurclaw's generated startup context, and optionally pass `initialItems` to seed V3 with complete role-bearing text messages at session creation. Version `"v1"` uses legacy Bidi `conversation.handoff.*`, `"v2"` uses the Realtime Voice API, and `"v3"` preserves V1 lemurclaw Voice behavior while using Frameless Bidi `delegation.*`. For V3 automatic lemurclaw text, `codexResponseHandoffMode` accepts `"thinking"` (the default; all output uses channel-less thinking appends), `"commentary"` (all output uses the commentary channel), or `"bemTags"` (the raw BEM envelope selects the API channel: BEM `analysis` and `commentary` use `commentary`, while BEM `final` and unparsable output use `speakable`). The BEM envelope remains in the appended text for the frontend model to interpret. V1 and V2 ignore this setting. V3 handoffs do not prepend the legacy `"Agent Final Message"` label. Pass `clientManagedHandoffs: true` to disable automatic lemurclaw response delivery so only the client's explicit append calls produce handoffs. Pass `codexResponsesAsItems: true` to send automatic lemurclaw responses as realtime conversation items instead, and optionally pass `codexResponseItemPrefix` to prepend experiment instructions to those items. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create a Bidi WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Conversation `version: "v2"` requests remain unsupported for WebRTC.
...
- `process/spawn` — experimental; spawn a standalone process without the lemurclaw sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new lemurclaw conversation.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current lemurclaw home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text, images, or audio) to a thread and trigger lemurclaw generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.lemurclaw/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a lemurclaw session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use lemurclaw's default realtime backend prompt. Send `prompt: null` or
...
Pass `includeStartupContext: false` to skip lemurclaw's startup context for this
...
`initialItems`.
Pass `clientManagedHandoffs: true` to suppress automatic lemurclaw response handoffs
...
Pass `codexResponsesAsItems: true` to inject automatic lemurclaw responses with
...
experiment instructions to each automatic lemurclaw response item. Omit
...
Use `review/start` to run lemurclaw’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions lemurclaw will run).
...
For an inline review, lemurclaw streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). lemurclaw will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, lemurclaw generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the lemurclaw sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a lemurclaw session/thread-group id.
...
- `contextCompaction` — `{id}` emitted when lemurclaw compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when lemurclaw compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`codexErrorInfo` maps to the `LemurclawErrorInfo` enum. Common values:
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell lemurclaw whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.lemurclaw/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a lemurclaw skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a lemurclaw skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.lemurclaw/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.lemurclaw/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.lemurclaw/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.lemurclaw/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.lemurclaw/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
lemurclaw supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`. Self-serve Business ProLite accounts use the `self_serve_business_prolite` plan type.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): lemurclaw owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; lemurclaw persists tokens to disk and refreshes them automatically.
- **lemurclaw managed Amazon Bedrock auth (`amazonBedrock`, experimental)**: Caller supplies an Amazon Bedrock API key and region via `account/login/start` with `type: "amazonBedrock"`. The client must enable the `experimentalApi` initialization capability for lemurclaw-managed Amazon Bedrock login. lemurclaw replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: lemurclaw uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `lemurclaw login --with-access-token` or `CODEX_ACCESS_TOKEN`.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, lemurclaw can run without OpenAI credentials.
- Amazon Bedrock reports `usesCodexManagedCredentials: true` when it uses a Bedrock API key managed by lemurclaw. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether lemurclaw-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted lemurclaw success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"lemurclaw"` or `"chatgpt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"lemurclaw"`.
...
1. Start:
   ...
   { "id": 4, "result": { "type": "chatgptDeviceCode", "loginId": "<uuid>", "verificationUrl": "https://auth.openai.com/lemurclaw/device", "userCode": "ABCD-1234" } }
   ```
...
When using a lemurclaw-managed Bedrock key, logout removes the key and clears `model_provider` if it is still set to `"amazon-bedrock"`. When using AWS-managed credentials, manage them through AWS or switch providers before logging out.
...
`lemurclaw app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
lemurclaw app-server generate-ts --out DIR --experimental
lemurclaw app-server generate-json-schema --out DIR --experimental
```
