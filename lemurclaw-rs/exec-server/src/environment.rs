...
pub const CODEX_EXEC_SERVER_URL_ENV_VAR: &str = "LEMURCLAW_EXEC_SERVER_URL";
pub const CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR: &str =
    "LEMURCLAW_EXEC_SERVER_NOISE_REGISTRY_URL";
pub const CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR: &str =
    "LEMURCLAW_EXEC_SERVER_NOISE_ENVIRONMENT_ID";
pub const CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR: &str = "LEMURCLAW_EXEC_SERVER_NOISE_AUTH_TOKEN";
pub const CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR: &str =
    "LEMURCLAW_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID";
...
mod tests {
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            lemurclaw_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = lemurclaw_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = lemurclaw_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            lemurclaw_protocol::models::PermissionProfile::from_runtime_permissions(
                &lemurclaw_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                lemurclaw_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
