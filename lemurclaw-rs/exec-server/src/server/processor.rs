    ...
    while let Some(event) = incoming_rx.recv().await {
        ...
        match event {
            JsonRpcConnectionEvent::MalformedMessage { reason } => {
                ...
                if outgoing_tx
                    .send(RpcServerOutboundMessage::Error {
                        request_id: lemurclaw_exec_server_protocol::RequestId::Integer(-1),
            ...
            JsonRpcConnectionEvent::Message(message) => match message {
                lemurclaw_exec_server_protocol::JSONRPCMessage::Request(request) => {
                ...
                lemurclaw_exec_server_protocol::JSONRPCMessage::Notification(notification) => {
                ...
                lemurclaw_exec_server_protocol::JSONRPCMessage::Response(response) => {
                ...
                lemurclaw_exec_server_protocol::JSONRPCMessage::Error(error) => {
...
fn request_span(
    span_name: &str,
    request: &lemurclaw_exec_server_protocol::JSONRPCRequest,
) -> tracing::Span {
    ...
    if let Some(trace) = &request.trace
        && !lemurclaw_otel::set_parent_from_w3c_trace_context(&span, trace)
    {
...
mod tests {
    ...
    use lemurclaw_exec_server_protocol::JSONRPCNotification;
    use lemurclaw_exec_server_protocol::JSONRPCRequest;
    ...
    fn request_span_uses_bounded_name_wire_method_and_inbound_trace_parent() {
        ...
        let subscriber = tracing_subscriber::registry().with(
            tracing_opentelemetry::layer()
                .with_tracer(tracer)
                .with_filter(filter_fn(lemurclaw_otel::OtelProvider::trace_export_filter)),
        );
        ...
        let trace = lemurclaw_protocol::protocol::W3cTraceContext {
