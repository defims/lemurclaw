...
use lemurclaw_test_binary_support::TestBinaryDispatchGuard;
use lemurclaw_test_binary_support::TestBinaryDispatchMode;
use lemurclaw_test_binary_support::configure_test_binary_dispatch;
use ctor::ctor;
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            lemurclaw_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(lemurclaw_exec_server::run_main(
        &listen_url,
        runtime_paths,
        http_client_factory,
    )) {
