...
use lemurclaw_exec_server::ExecBackend;
use lemurclaw_exec_server::ExecOutputStream;
use lemurclaw_exec_server::ExecParams;
use lemurclaw_exec_server::ExecProcess;
use lemurclaw_exec_server::ExecProcessEvent;
#[cfg(any(unix, windows))]
use lemurclaw_exec_server::FileSystemSandboxContext;
use lemurclaw_exec_server::ProcessId;
use lemurclaw_exec_server::ProcessSignal;
use lemurclaw_exec_server::ReadResponse;
use lemurclaw_exec_server::StartedExecProcess;
use lemurclaw_exec_server::WriteStatus;
...
async fn remote_sandboxed_process_preserves_custom_arg0() -> Result<()> {
    if let Some(warning) = lemurclaw_sandboxing::system_bwrap_warning(&PermissionProfile::read_only()) {
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            env: HashMap::from([
                ...
                (
                    "LEMURCLAW_TEST_DENIED_FILE".to_string(),
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("lemurclaw-linux-sandbox");
