...
use lemurclaw_exec_server::EnvironmentStatus;
use lemurclaw_exec_server::EnvironmentStatusKind;
...
async fn exec_server_serves_readyz_alongside_websocket_endpoint() -> anyhow::Result<()> {
    ...
    let client = lemurclaw_http_client::HttpClientBuilder::new().build_direct()?;
    let response = client
