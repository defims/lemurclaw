...
use lemurclaw_api::ResponsesClient;
use lemurclaw_api::ResponsesOptions;
use lemurclaw_client::HttpTransport;
use lemurclaw_client::Request;
use lemurclaw_client::RequestBody;
use lemurclaw_client::Response;
use lemurclaw_client::StreamResponse;
use lemurclaw_client::TransportError;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: lemurclaw_api::RetryConfig {
            max_attempts: 1,
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, lemurclaw_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, lemurclaw_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> lemurclaw_api::AuthProviderFuture<'_> {
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, lemurclaw_client::RequestCompression::None);
    Ok(())
}
