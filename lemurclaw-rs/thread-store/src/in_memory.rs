...
use lemurclaw_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: metadata
            ...
            .map(|id| lemurclaw_state::ThreadSection {
                name: if id == lemurclaw_state::PINNED_THREAD_SECTION_ID {
                    lemurclaw_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<lemurclaw_protocol::protocol::GitInfo> {
    ...
    Some(lemurclaw_protocol::protocol::GitInfo {
