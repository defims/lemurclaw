...
use lemurclaw_protocol::protocol::user_message_preview;
...
impl ThreadMetadataSync {
    ...
    pub(crate) fn observe_appended_items(
        ...
        let affects_metadata = items
            .iter()
            .any(lemurclaw_state::rollout_item_affects_thread_metadata);
...
mod tests {
    ...
    fn resume_preserves_existing_display_metadata_and_initializes_missing_title() {
        let thread_id = ThreadId::new();
        let mut metadata = lemurclaw_state::ThreadMetadataBuilder::new(
            thread_id,
            Default::default(),
            Utc::now(),
            SessionSource::Exec,
