...
use lemurclaw_rollout::RolloutRecorderParams;
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = lemurclaw_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = lemurclaw_state::open_thread_history_db(&lemurclaw_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn catch_up_rejects_invalid_complete_suffixes_without_advancing_state() {
    ...
    for (name, suffix) in cases {
        ...
        let pool = lemurclaw_state::open_thread_history_db(
