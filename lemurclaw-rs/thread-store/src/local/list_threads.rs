...
use lemurclaw_rollout::RolloutConfig;
use lemurclaw_rollout::RolloutRecorder;
use lemurclaw_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => lemurclaw_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => lemurclaw_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => lemurclaw_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => lemurclaw_rollout::SortDirection::Asc,
        SortDirection::Desc => lemurclaw_rollout::SortDirection::Desc,
    };
...
pub(super) async fn list_rollout_threads(
    state_db: Option<lemurclaw_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&lemurclaw_rollout::Cursor>,
    sort_key: lemurclaw_rollout::ThreadSortKey,
    sort_direction: lemurclaw_rollout::SortDirection,
) -> ThreadStoreResult<lemurclaw_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    lemurclaw_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    lemurclaw_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = lemurclaw_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
