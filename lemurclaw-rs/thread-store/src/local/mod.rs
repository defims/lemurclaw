...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl lemurclaw_rollout::RolloutConfigView) -> Self {
        Self {
...
impl LocalThreadStore {
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        ...
        self.thread_history_db
            .get_or_try_init(|| async {
                lemurclaw_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
    ...
    async fn acquire_paginated_writer_locks(
        ...
        for &thread_id in thread_ids {
            ...
            .await?
            {
                Some(rollout_path) => lemurclaw_rollout::read_session_meta_line(rollout_path.as_path())
                    .await
                    .ok()
        ...
        live_thread
            .append_items(&[
                ...
                RolloutItem::EventMsg(EventMsg::TokenCount(
                    lemurclaw_protocol::protocol::TokenCountEvent {
                        info: None,
                        rate_limits: None,
                    },
                )),
