...
use lemurclaw_rollout::RolloutReferenceIndex;
use lemurclaw_rollout::SESSIONS_SUBDIR;
use lemurclaw_rollout::find_archived_thread_path_by_id_str;
use lemurclaw_rollout::find_thread_path_by_id_str;
use lemurclaw_rollout::remove_thread_name_entries;
...
fn referenced_thread_error(thread_id: lemurclaw_protocol::ThreadId) -> ThreadStoreError {
...
fn delete_rollout_file(
    ...
    let plain_path = lemurclaw_rollout::plain_rollout_path(rollout_path);
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = lemurclaw_state::open_thread_history_db(&config.sqlite)
            .await
