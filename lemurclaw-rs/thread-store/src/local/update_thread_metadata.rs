...
use lemurclaw_protocol::protocol::GitInfo;
...
use lemurclaw_rollout::ARCHIVED_SESSIONS_SUBDIR;
use lemurclaw_rollout::append_rollout_item_to_path;
use lemurclaw_rollout::append_thread_name;
use lemurclaw_rollout::find_archived_thread_path_by_id_str;
use lemurclaw_rollout::find_thread_path_by_id_str;
...
pub(super) async fn update_thread_metadata(
    ...
    lemurclaw_rollout::state_db::reconcile_rollout(
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = lemurclaw_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
        ...
        async {
            ...
            if existing.is_none()
                ...
                && let Some(existing_rollout_path) =
                    lemurclaw_rollout::existing_rollout_path(path).await
            {
                lemurclaw_rollout::state_db::reconcile_rollout(
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<lemurclaw_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if lemurclaw_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    lemurclaw_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(lemurclaw_git_utils::GitSha::new),
        branch: branch.clone(),
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = lemurclaw_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn section_only_metadata_updates_persist_in_sqlite_without_changing_the_rollout() {
        ...
        let runtime = lemurclaw_state::StateRuntime::init(
            config.sqlite.clone(),
        ...
        let pinned = store
            .update_thread_metadata(UpdateThreadMetadataParams {
                thread_id,
                patch: ThreadMetadataPatch {
                    section: Some(Some(lemurclaw_state::PINNED_THREAD_SECTION_ID.to_string())),
                    ..Default::default()
                },
                include_archived: false,
            })
        ...
        assert_eq!(
            pinned.section,
            Some(lemurclaw_state::ThreadSection {
                id: lemurclaw_state::PINNED_THREAD_SECTION_ID.to_string(),
                name: lemurclaw_state::PINNED_THREAD_SECTION_NAME.to_string(),
            })
        );
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            lemurclaw_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
    ...
    async fn update_thread_metadata_sets_memory_mode_on_active_rollout() {
        ...
        let runtime = lemurclaw_state::StateRuntime::init(
            lemurclaw_state::SqliteConfig::new_for_testing(home.path().abs()),
            config.default_model_provider_id.clone(),
        )
        .await
        .expect("state db should initialize");
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd = lemurclaw_utils_path::normalize_for_path_comparison(workspace.as_path())
