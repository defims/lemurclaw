...
use lemurclaw_rollout::rollout_date_parts;
...
pub(super) async fn unarchive_thread(
    ...
    let canonical_archived_path = scoped_rollout_path(
        store
            .config
            .codex_home
            .join(lemurclaw_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ...
    let dest_dir = store
        .config
        .codex_home
        .join(lemurclaw_rollout::SESSIONS_SUBDIR)
        .join(year)
        .join(month)
        .join(day);
...
mod tests {
    ...
    async fn unarchive_thread_updates_sqlite_metadata_when_present() {
        ...
        metadata.section = Some(lemurclaw_state::ThreadSection {
