...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if lemurclaw_rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let Some(path) = lemurclaw_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && lemurclaw_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(lemurclaw_rollout::plain_rollout_path(path.as_path()));
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<lemurclaw_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if lemurclaw_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
    ...
    let rollout_path = lemurclaw_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = lemurclaw_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        lemurclaw_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
    ...
    async fn read_paginated_thread_prefers_sqlite_display_metadata() {
        ...
        .expect("session file");
        lemurclaw_rollout::append_rollout_item_to_path(
            &rollout_path,
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        lemurclaw_rollout::append_thread_name(home.path(), thread_id, "Stale index name")
            .await
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        lemurclaw_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
