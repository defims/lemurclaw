use lemurclaw_config::HooksFile;
...
use lemurclaw_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use lemurclaw_utils_plugins::find_plugin_manifest_path;
...
pub type PluginManifest = lemurclaw_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = lemurclaw_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = lemurclaw_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    lemurclaw_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = lemurclaw_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = lemurclaw_plugin::manifest::PluginManifest<PathUri>;
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = lemurclaw_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(lemurclaw_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: lemurclaw_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<lemurclaw_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| lemurclaw_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(lemurclaw_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(lemurclaw_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(lemurclaw_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<lemurclaw_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(lemurclaw_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(lemurclaw_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use lemurclaw_plugin::PluginProvider;
    use lemurclaw_plugin::ResolvedPlugin;
    use lemurclaw_plugin::manifest::PluginManifest as GenericPluginManifest;
    use lemurclaw_plugin::manifest::PluginManifestHooks;
    use lemurclaw_plugin::manifest::PluginManifestInterface;
    use lemurclaw_plugin::manifest::PluginManifestMcpServers;
    use lemurclaw_plugin::manifest::PluginManifestPaths;
