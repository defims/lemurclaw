...
use lemurclaw_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == lemurclaw_features::Feature::Artifact {
            continue;
        }
        if feature.id == lemurclaw_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == lemurclaw_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == lemurclaw_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<lemurclaw_features::FeatureToml<
                    lemurclaw_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
