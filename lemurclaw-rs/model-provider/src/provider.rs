...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    fn map_api_error(&self, error: ApiError) -> CodexErr {
        lemurclaw_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, lemurclaw_protocol::error::Result<Provider>> {
        Box::pin(async move {
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, lemurclaw_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, lemurclaw_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    ) -> ModelProviderFuture<'_, lemurclaw_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| lemurclaw_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
...
mod tests {
    ...
    use lemurclaw_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = lemurclaw_models_manager::bundled_models_response()
