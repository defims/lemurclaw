...
use lemurclaw_app_server_protocol::DynamicToolCallOutputContentItem;
...
use lemurclaw_app_server_protocol::WebSearchAction;
use lemurclaw_git_utils::collect_git_info;
...
use lemurclaw_protocol::items::is_safe_plugin_relative_path;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn tool_item(
        notification: &'a lemurclaw_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
impl AnalyticsReducer {
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: lemurclaw_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
...
fn file_change_counts(changes: &[lemurclaw_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn analytics_turn_status(status: lemurclaw_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        lemurclaw_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        lemurclaw_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        lemurclaw_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        lemurclaw_app_server_protocol::TurnStatus::InProgress => None,
    }
}
