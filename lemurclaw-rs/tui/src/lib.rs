...
use lemurclaw_app_server_client::RemoteAppServerClient;
use lemurclaw_app_server_client::RemoteAppServerConnectArgs;
pub use lemurclaw_app_server_client::RemoteAppServerEndpoint;
use lemurclaw_app_server_protocol::Account as AppServerAccount;
...
use lemurclaw_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use lemurclaw_exec_server::ExecServerRuntimePaths;
...
use lemurclaw_rollout::StateDbHandle;
use lemurclaw_rollout::state_db;
use lemurclaw_state::log_db;
...
use lemurclaw_utils_home_dir::find_codex_home;
...
pub(crate) use lemurclaw_app_server_client::legacy_core;
...
async fn start_embedded_app_server(
    ...
    feedback: lemurclaw_feedback::CodexFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = lemurclaw_state::runtime_db_path_for_corruption_error(&err)
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            ...
            lemurclaw_app_server_client::app_server_control_socket_path(&codex_home)
...
async fn maybe_probe_default_daemon_socket(codex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = lemurclaw_app_server_client::app_server_control_socket_path(codex_home).ok()?;
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        lemurclaw_feedback::CodexFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&lemurclaw_state::StateRuntime>,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = lemurclaw_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        lemurclaw_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(lemurclaw_otel::OtelProvider::metrics) {
        let _ = lemurclaw_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            lemurclaw_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = lemurclaw_state::install_process_db_telemetry(telemetry);
    }
    ...
    let feedback = lemurclaw_feedback::CodexFeedback::new();
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        lemurclaw_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let session_meta = lemurclaw_protocol::protocol::SessionMeta {
            ...
            source: lemurclaw_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(lemurclaw_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let cloud_config_bundle =
                lemurclaw_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: lemurclaw_app_server_client::app_server_control_socket_path(&codex_home)?,
            }
        );
        Ok(())
    }
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codex_home = TempDir::new()?;
        let socket_path =
            lemurclaw_app_server_client::app_server_control_socket_path(codex_home.path())?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            lemurclaw_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = lemurclaw_state::StateRuntime::init(
                lemurclaw_state::SqliteConfig::new_for_testing(config.codex_home.as_path().abs()),
            ...
            let mut builder = lemurclaw_state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            lemurclaw_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = lemurclaw_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            lemurclaw_state::sqlite_error_detail_is_corruption(startup_error.detail()),
