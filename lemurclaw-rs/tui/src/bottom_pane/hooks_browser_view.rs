...
impl HooksBrowserView {
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        lemurclaw_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| lemurclaw_protocol::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => lemurclaw_protocol::protocol::HookEventName::iter().count(),
...
mod tests {
    ...
    use lemurclaw_app_server_protocol::HookErrorInfo;
    use lemurclaw_app_server_protocol::HookEventName;
