...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = lemurclaw_config::ConfigRequirementsToml {
        windows: Some(lemurclaw_config::WindowsRequirementsToml {
...
fn requirements_stack(requirements_toml: lemurclaw_config::ConfigRequirementsToml) -> ConfigLayerStack {
    let mut requirements_with_sources = lemurclaw_config::ConfigRequirementsWithSources::default();
    requirements_with_sources
        .merge_unset_fields(RequirementSource::Unknown, requirements_toml.clone());
    let requirements = lemurclaw_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(lemurclaw_config::ConfigRequirementsToml {
