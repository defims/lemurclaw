...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![lemurclaw_utils_path_uri::PathUri::from_abs_path(
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(lemurclaw_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(lemurclaw_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(lemurclaw_config::types::KeybindingsSpec::One(
        lemurclaw_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            lemurclaw_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: lemurclaw_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: lemurclaw_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
