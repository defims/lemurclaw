...
pub(super) async fn test_config() -> Config {
    ...
    config.sqlite = lemurclaw_state::SqliteConfig::new_for_testing(codex_home.as_path().abs());
...
fn token_usage_breakdown(usage: TokenUsage) -> lemurclaw_app_server_protocol::TokenUsageBreakdown {
    lemurclaw_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    lemurclaw_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: lemurclaw_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = lemurclaw_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: lemurclaw_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            lemurclaw_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(lemurclaw_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    lemurclaw_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: lemurclaw_app_server_protocol::HookEventName,
    status: lemurclaw_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<lemurclaw_app_server_protocol::HookOutputEntry>,
) -> lemurclaw_app_server_protocol::HookRunSummary {
    lemurclaw_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: lemurclaw_app_server_protocol::HookHandlerType::Command,
        execution_mode: lemurclaw_app_server_protocol::HookExecutionMode::Sync,
        scope: lemurclaw_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: lemurclaw_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            lemurclaw_app_server_protocol::HookRunStatus::Completed
                | lemurclaw_app_server_protocol::HookRunStatus::Failed
                | lemurclaw_app_server_protocol::HookRunStatus::Blocked
                | lemurclaw_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            lemurclaw_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                lemurclaw_app_server_protocol::HookOutputEntry {
                    kind: lemurclaw_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                lemurclaw_app_server_protocol::HookOutputEntry {
                    kind: lemurclaw_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: lemurclaw_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        lemurclaw_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        lemurclaw_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        lemurclaw_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        lemurclaw_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        lemurclaw_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        lemurclaw_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        lemurclaw_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        lemurclaw_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        lemurclaw_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        lemurclaw_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        lemurclaw_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
