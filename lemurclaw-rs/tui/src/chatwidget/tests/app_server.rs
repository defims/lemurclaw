...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> lemurclaw_app_server_protocol::ThreadSettingsUpdatedNotification {
    lemurclaw_app_server_protocol::ThreadSettingsUpdatedNotification {
        thread_id: thread_id.to_string(),
        thread_settings: lemurclaw_app_server_protocol::ThreadSettings {
...
async fn invalid_url_elicitation_is_declined() {
    ...
    chat.handle_elicitation_request_now(
        lemurclaw_app_server_protocol::RequestId::Integer(9),
        lemurclaw_app_server_protocol::McpServerElicitationRequestParams {
            thread_id: request_thread_id.to_string(),
            turn_id: Some("turn-auth".to_string()),
            server_name: "payments".to_string(),
            request: lemurclaw_app_server_protocol::McpServerElicitationRequest::Url {
                meta: None,
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: lemurclaw_app_server_protocol::RequestId::Integer(9),
...
async fn thread_settings_updated_updates_visible_state_without_transcript() {
    ...
    assert_eq!(
        chat.config_ref()
            ...
            .id,
        lemurclaw_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
    );
...
async fn live_app_server_turn_completion_repairs_dropped_message_deltas() {
    ...
    completed_turn.items_view = lemurclaw_app_server_protocol::TurnItemsView::Summary;
