...
use lemurclaw_app_server_client::AppServerClient;
use lemurclaw_app_server_client::AppServerEvent;
use lemurclaw_app_server_client::AppServerPath;
use lemurclaw_app_server_client::AppServerRequestHandle;
use lemurclaw_app_server_client::TypedRequestError;
...
use lemurclaw_app_server_protocol::ConfigRequirementsReadResponse;
use lemurclaw_app_server_protocol::ConfigWriteResponse;
use lemurclaw_app_server_protocol::ExternalAgentConfigDetectParams;
use lemurclaw_app_server_protocol::ExternalAgentConfigDetectResponse;
use lemurclaw_app_server_protocol::ExternalAgentConfigImportParams;
use lemurclaw_app_server_protocol::ExternalAgentConfigImportResponse;
use lemurclaw_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use lemurclaw_app_server_protocol::Model as ApiModel;
use lemurclaw_app_server_protocol::ModelListParams;
use lemurclaw_app_server_protocol::ModelListResponse;
use lemurclaw_app_server_protocol::NewThreadModelDefaults;
use lemurclaw_app_server_protocol::RateLimitSnapshot;
use lemurclaw_app_server_protocol::RequestId;
use lemurclaw_app_server_protocol::ReviewDelivery;
use lemurclaw_app_server_protocol::ReviewStartParams;
use lemurclaw_app_server_protocol::ReviewStartResponse;
use lemurclaw_app_server_protocol::ReviewTarget;
use lemurclaw_app_server_protocol::SessionSource;
...
use lemurclaw_app_server_protocol::ThreadMemoryMode;
...
use lemurclaw_protocol::approvals::GuardianAssessmentEvent;
...
use lemurclaw_protocol::openai_models::ModelAvailabilityNux;
use lemurclaw_protocol::openai_models::ModelPreset;
use lemurclaw_protocol::openai_models::ModelServiceTier;
use lemurclaw_protocol::openai_models::ModelUpgrade;
use lemurclaw_protocol::openai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<lemurclaw_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: lemurclaw_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<lemurclaw_protocol::openai_models::ReasoningEffort>,
        summary: Option<lemurclaw_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<lemurclaw_protocol::config_types::CollaborationMode>,
        personality: Option<lemurclaw_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<lemurclaw_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<lemurclaw_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<lemurclaw_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(lemurclaw_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(lemurclaw_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(lemurclaw_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(lemurclaw_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<lemurclaw_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &lemurclaw_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<lemurclaw_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        lemurclaw_message_history::HistoryConfig::new(config.codex_home.clone(), &config.history);
    let (log_id, entry_count) = lemurclaw_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(lemurclaw_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(lemurclaw_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn side_fork_excludes_turns_without_clearing_regular_ephemeral_fork() -> Result<()> {
        ...
        assert!(matches!(
            ...
            [lemurclaw_app_server_protocol::ThreadItem::UserMessage { content, .. }]
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: lemurclaw_app_server_protocol::Thread {
                ...
                source: lemurclaw_app_server_protocol::SessionSource::Cli,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        lemurclaw_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![lemurclaw_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        lemurclaw_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: lemurclaw_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: lemurclaw_app_server_protocol::ApprovalsReviewer::User,
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &lemurclaw_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        lemurclaw_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        lemurclaw_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            lemurclaw_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(lemurclaw_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(lemurclaw_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let business_prolite = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(lemurclaw_protocol::account::PlanType::SelfServeBusinessProLite),
        );
        assert!(matches!(
            business_prolite,
            Some(StatusAccountDisplay::ChatGpt {
