...
use lemurclaw_ansi_escape::ansi_escape_line;
...
use lemurclaw_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use lemurclaw_app_server_protocol::SkillErrorInfo;
...
use lemurclaw_app_server_protocol::WriteStatus;
...
use lemurclaw_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use lemurclaw_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use lemurclaw_protocol::permissions::FileSystemSandboxKind;
...
use lemurclaw_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        lemurclaw_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&lemurclaw_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&lemurclaw_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[lemurclaw_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&lemurclaw_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<lemurclaw_app_server_protocol::CommandExecutionApprovalDecision> {
    use lemurclaw_app_server_protocol::CommandExecutionApprovalDecision;
    use lemurclaw_app_server_protocol::NetworkPolicyRuleAction;
...
fn resume_hint_for_resumable_thread(
    ...
    lemurclaw_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
impl App {
    ...
    pub async fn run(
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            lemurclaw_login::default_client::originator().value,
