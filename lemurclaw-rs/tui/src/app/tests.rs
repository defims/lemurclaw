...
use lemurclaw_utils_absolute_path::test_support::PathExt;
...
use lemurclaw_app_server_client::AppServerPath;
use lemurclaw_app_server_protocol::AdditionalFileSystemPermissions;
use lemurclaw_app_server_protocol::AdditionalNetworkPermissions;
use lemurclaw_app_server_protocol::AdditionalPermissionProfile;
use lemurclaw_app_server_protocol::AgentMessageDeltaNotification;
...
use lemurclaw_app_server_protocol::FileUpdateChange;
use lemurclaw_app_server_protocol::ItemStartedNotification;
use lemurclaw_app_server_protocol::JSONRPCErrorError;
use lemurclaw_app_server_protocol::McpServerElicitationRequest;
...
use lemurclaw_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use lemurclaw_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use lemurclaw_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use lemurclaw_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use lemurclaw_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use lemurclaw_app_server_protocol::PatchChangeKind;
use lemurclaw_app_server_protocol::PermissionsRequestApprovalParams;
use lemurclaw_app_server_protocol::RequestId as AppServerRequestId;
...
use lemurclaw_app_server_protocol::TokenUsageBreakdown;
...
use lemurclaw_app_server_protocol::TurnError as AppServerTurnError;
use lemurclaw_app_server_protocol::TurnStartedNotification;
use lemurclaw_app_server_protocol::TurnStatus;
use lemurclaw_app_server_protocol::UserInput;
use lemurclaw_app_server_protocol::UserInput as AppServerUserInput;
...
use lemurclaw_protocol::config_types::CollaborationModeMask;
use lemurclaw_protocol::config_types::ModeKind;
use lemurclaw_protocol::config_types::Personality;
use lemurclaw_protocol::config_types::SandboxMode;
...
use lemurclaw_protocol::models::NetworkPermissions;
use lemurclaw_protocol::models::PermissionProfile;
use lemurclaw_protocol::protocol::EventMsg;
use lemurclaw_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
use lemurclaw_protocol::protocol::SessionSource as RolloutSessionSource;
...
use lemurclaw_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let lemurclaw_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: lemurclaw_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: lemurclaw_feedback::CodexFeedback::new(),
        is_first_run: false,
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = lemurclaw_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = lemurclaw_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(lemurclaw_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: lemurclaw_app_server_protocol::CollabAgentTool::Wait,
                status: lemurclaw_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(lemurclaw_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: lemurclaw_app_server_protocol::CollabAgentTool::SendInput,
                status: lemurclaw_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    lemurclaw_app_server_protocol::CollabAgentState {
                        status: lemurclaw_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let codex_home = tempdir()?;
        app.config.codex_home = codex_home.path().to_path_buf().abs();
        app.config.sqlite = lemurclaw_state::SqliteConfig::new_for_testing(codex_home.path().abs());
        ...
        let state_db = lemurclaw_state::StateRuntime::init(
            lemurclaw_state::SqliteConfig::new_for_testing(codex_home.path().abs()),
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: lemurclaw_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            lemurclaw_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            lemurclaw_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            lemurclaw_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            lemurclaw_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: lemurclaw_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: lemurclaw_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: lemurclaw_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: lemurclaw_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: lemurclaw_app_server_protocol::SessionSource::Unknown,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            lemurclaw_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        lemurclaw_app_server_client::AppServerEvent::ServerNotification(Box::new(
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        lemurclaw_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(Op::ExecApproval {
            ...
            turn_id: None,
            decision: lemurclaw_app_server_protocol::CommandExecutionApprovalDecision::Accept,
        })
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            lemurclaw_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        lemurclaw_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
fn test_turn(turn_id: &str, status: TurnStatus, items: Vec<ThreadItem>) -> Turn {
    Turn {
        ...
        items_view: lemurclaw_app_server_protocol::TurnItemsView::Full,
        items,
        status,
        error: None,
        started_at: None,
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(lemurclaw_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(lemurclaw_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            lemurclaw_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    lemurclaw_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: lemurclaw_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                lemurclaw_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                lemurclaw_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: lemurclaw_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: lemurclaw_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                lemurclaw_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
