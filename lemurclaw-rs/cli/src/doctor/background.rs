...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let socket_path = match lemurclaw_app_server::app_server_control_socket_path(&config.codex_home) {
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match lemurclaw_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    fn create_socket_placeholder(config: &Config) {
        let socket_path = lemurclaw_app_server::app_server_control_socket_path(&config.codex_home)
            .expect("socket path");
