...
use lemurclaw_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let rows = match lemurclaw_state::read_thread_state_audit_rows(sqlite).await {
        Ok(rows) => rows,
        Err(err) => {
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = lemurclaw_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match lemurclaw_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    let logical_path = lemurclaw_rollout::plain_rollout_path(path);
    lemurclaw_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&lemurclaw_rollout::plain_rollout_path(path))
}
...
mod tests {
    ...
    impl Fixture {
        async fn new() -> Self {
            ...
            let _runtime = lemurclaw_state::StateRuntime::init(
        ...
        fn sqlite(&self) -> lemurclaw_state::SqliteConfig {
            lemurclaw_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
        }
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(lemurclaw_protocol::protocol::SessionMetaLine {
