...
use lemurclaw_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use lemurclaw_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use lemurclaw_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use lemurclaw_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use lemurclaw_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use lemurclaw_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = lemurclaw_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = lemurclaw_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = lemurclaw_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: lemurclaw_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(lemurclaw_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    lemurclaw_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
