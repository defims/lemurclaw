...
use lemurclaw_state::RuntimeDbBackup;
use lemurclaw_tui::LocalStateDbStartupError;
...
pub(crate) fn is_locked(detail: &str) -> bool {
    lemurclaw_state::sqlite_error_detail_is_lock(detail)
}

pub(crate) fn is_corruption(detail: &str) -> bool {
    lemurclaw_state::sqlite_error_detail_is_corruption(detail)
}
...
pub(crate) async fn backup_files_for_fresh_start(
    ...
    lemurclaw_state::backup_runtime_db_for_fresh_start(startup_error.database_path()).await
}
...
mod tests {
    ...
    async fn backup_backs_up_only_failed_database_file() -> std::io::Result<()> {
        ...
        let sqlite = lemurclaw_state::SqliteConfig::new_for_testing(temp_dir.path().abs());
