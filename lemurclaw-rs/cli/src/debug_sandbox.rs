...
#[cfg(target_os = "macos")]
use lemurclaw_core::spawn::CODEX_SANDBOX_ENV_VAR;
use lemurclaw_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use lemurclaw_sandboxing::landlock::allow_network_for_proxy;
use lemurclaw_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use lemurclaw_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use lemurclaw_sandboxing::seatbelt::create_seatbelt_command_args;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<lemurclaw_mcp::SandboxState>)
        .transpose()
    ...
    let network = network_proxy
        .as_ref()
        .map(lemurclaw_core::config::StartedNetworkProxy::proxy);
...
async fn run_command_under_windows_session(
    ...
    use lemurclaw_windows_sandbox::WindowsSandboxProxySettingsMode;
    use lemurclaw_windows_sandbox::WindowsSandboxSessionRequest;
    use lemurclaw_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let exit_code = lemurclaw_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
"#;
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = lemurclaw_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
