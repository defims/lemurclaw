...
use lemurclaw_login::ServerOptions;
use lemurclaw_login::login_with_access_token;
use lemurclaw_login::login_with_api_key;
use lemurclaw_login::logout_with_revoke;
use lemurclaw_login::run_device_code_login;
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match lemurclaw_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
