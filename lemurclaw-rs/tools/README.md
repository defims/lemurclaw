...
`lemurclaw-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `lemurclaw-core`.
...
compatibility-sensitive orchestration in `lemurclaw-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `lemurclaw-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `lemurclaw-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `lemurclaw-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `lemurclaw-core`.
3. Leave compatibility-sensitive adapters in `lemurclaw-core` while downstream
...
`lemurclaw-core`, that is a sign to revisit the extraction boundary before adding
more here.
