...
> **Privacy:** Rollout tracing is not telemetry. lemurclaw does **not** upload or
...
during a lemurclaw session. It records raw runtime evidence into a local bundle on
...
Hot-path lemurclaw code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how lemurclaw got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["lemurclaw-core runtime"]
...
lemurclaw session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `lemurclaw debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
lemurclaw rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
lemurclaw debug trace-reduce <trace-bundle>
```
