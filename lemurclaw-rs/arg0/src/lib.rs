...
use lemurclaw_apply_patch::CODEX_CORE_APPLY_PATCH_ARG1;
#[cfg(unix)]
use lemurclaw_exec_server::CODEX_ARG0_EXEC_HELPER_ARG1;
use lemurclaw_exec_server::CODEX_FS_HELPER_ARG1;
use lemurclaw_install_context::InstallContext;
use lemurclaw_sandboxing::landlock::CODEX_LINUX_SANDBOX_ARG0;
use lemurclaw_utils_home_dir::find_codex_home;
#[cfg(target_os = "windows")]
use lemurclaw_windows_sandbox::CODEX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            lemurclaw_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == CODEX_LINUX_SANDBOX_ARG0 {
        ...
        lemurclaw_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        lemurclaw_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == CODEX_ARG0_EXEC_HELPER_ARG1 {
        lemurclaw_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == CODEX_FS_HELPER_ARG1 {
        lemurclaw_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == CODEX_WINDOWS_SANDBOX_ARG1 {
        lemurclaw_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                let cwd = match lemurclaw_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                match runtime.block_on(lemurclaw_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    lemurclaw_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
