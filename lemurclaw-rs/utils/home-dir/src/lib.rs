...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    let codex_home_env = std::env::var("LEMURCLAW_HOME")
        .ok()
        ...
        Some(val) => {
            ...
            let metadata = std::fs::metadata(&path).map_err(|err| match err.kind() {
                std::io::ErrorKind::NotFound => std::io::Error::new(
                    ...
                    format!("LEMURCLAW_HOME points to {val:?}, but that path does not exist"),
                ),
                _ => std::io::Error::new(
                    err.kind(),
            ...
            if !metadata.is_dir() {
                Err(std::io::Error::new(
                    ...
                    format!("LEMURCLAW_HOME points to {val:?}, but that path is not a directory"),
                ))
            } else {
...
mod tests {
    ...
    fn find_codex_home_env_missing_path_is_fatal() {
        ...
        assert!(
            err.to_string().contains("LEMURCLAW_HOME"),
