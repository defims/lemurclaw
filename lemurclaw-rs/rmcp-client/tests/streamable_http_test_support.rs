...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    lemurclaw_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn control_client() -> anyhow::Result<lemurclaw_http_client::HttpClient> {
    Ok(lemurclaw_http_client::HttpClientBuilder::new().build_direct()?)
}
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(lemurclaw_utils_cargo_bin::cargo_bin("codex")?)
