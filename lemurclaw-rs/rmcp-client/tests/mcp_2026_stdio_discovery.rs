...
-> anyhow::Result<()> {
    let server = lemurclaw_utils_cargo_bin::cargo_bin("test_mcp_2026_discovery_stdio_server")?;
    ...
    for executor in [false, true] {
        ...
        let client = RmcpClient::new_stdio_client_with_protocol_mode(
            ...
            Vec::new(),
            Some(HashMap::from([(
                OsString::from("LEMURCLAW_MCP_PROTOCOL_VERSION"),
...
async fn legacy_stdio_preserves_existing_protocol_marker_environment() -> anyhow::Result<()> {
    let server = lemurclaw_utils_cargo_bin::cargo_bin("test_stdio_server")?;
            ...
            let result = client
                .call_tool(
                    ...
                    Some(json!({
                        ...
                        "env_var": "LEMURCLAW_MCP_PROTOCOL_VERSION",
                    })),
                    /*meta*/ None,
                    Some(Duration::from_secs(10)),
                )
