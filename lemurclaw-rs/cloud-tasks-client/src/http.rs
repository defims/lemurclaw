...
use lemurclaw_backend_client as backend;
use lemurclaw_backend_client::CodeTaskDetailsResponseExt;
use lemurclaw_git_utils::ApplyGitRequest;
use lemurclaw_git_utils::apply_git_patch;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("LEMURCLAW_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
