...
use lemurclaw_backend_client::ConfigBundleResponse;
use lemurclaw_backend_client::DeliveredTomlFragment;
use lemurclaw_config::AbsolutePathBuf;
use lemurclaw_config::CloudConfigFragment;
use lemurclaw_config::CloudConfigTomlBundle;
use lemurclaw_config::CloudRequirementsFragment;
use lemurclaw_config::CloudRequirementsTomlBundle;
...
use lemurclaw_login::auth::ExternalAuth;
use lemurclaw_login::auth::ExternalAuthRefreshContext;
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    let key_material =
        lemurclaw_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
impl ExternalAuth for TestExternalChatgptAuth {
    fn resolve(&self) -> lemurclaw_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async {
            self.current
                .read()
                .map(|auth| auth.clone())
    ...
    ) -> lemurclaw_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async {
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(lemurclaw_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            lemurclaw_backend_client::DeliveredRequirementsToml {
