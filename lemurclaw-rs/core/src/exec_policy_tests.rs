...
use lemurclaw_config::RequirementsExecPolicy;
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        lemurclaw_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(lemurclaw_config::Sourced::new(
            lemurclaw_config::RequirementsExecPolicy::new(requirements_exec_policy),
            lemurclaw_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(lemurclaw_config::CONFIG_TOML_FILE))
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let root_rules = project_root.join(".lemurclaw").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".lemurclaw").join(RULES_DIR_NAME);
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".lemurclaw").join(RULES_DIR_NAME);
