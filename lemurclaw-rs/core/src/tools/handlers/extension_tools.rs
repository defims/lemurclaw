...
use lemurclaw_tools::ConversationHistory;
use lemurclaw_tools::ExtensionTurnItem;
use lemurclaw_tools::ToolCall as ExtensionToolCall;
use lemurclaw_tools::ToolEnvironment;
use lemurclaw_tools::ToolName;
use lemurclaw_tools::ToolSearchInfo;
use lemurclaw_tools::ToolSpec;
use lemurclaw_tools::TurnItemEmissionFuture;
use lemurclaw_tools::TurnItemEmitter;
use lemurclaw_utils_string::to_ascii_json_string;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn lemurclaw_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn lemurclaw_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
impl ToolExecutor<ToolInvocation> for ExtensionToolAdapter {
    ...
    fn handle(&self, invocation: ToolInvocation) -> lemurclaw_tools::ToolExecutorFuture<'_> {
...
mod tests {
    ...
    use lemurclaw_extension_items::web_search::WebSearchItem;
    use lemurclaw_protocol::items::TurnItem;
    ...
    impl lemurclaw_extension_api::ToolExecutor<lemurclaw_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> lemurclaw_tools::ToolName {
            lemurclaw_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> lemurclaw_tools::ToolSpec {
            lemurclaw_tools::ToolSpec::Function(lemurclaw_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: lemurclaw_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: lemurclaw_tools::ToolCall) -> lemurclaw_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(lemurclaw_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn lemurclaw_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<lemurclaw_tools::ToolCall>>>,
    }

    impl lemurclaw_extension_api::ToolExecutor<lemurclaw_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> lemurclaw_tools::ToolName {
            lemurclaw_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> lemurclaw_tools::ToolSpec {
            lemurclaw_tools::ToolSpec::Function(lemurclaw_tools::ResponsesApiTool {
                ...
                strict: false,
                parameters: lemurclaw_tools::JsonSchema::default(),
                output_schema: None,
                defer_loading: None,
            })
        }

        fn handle(&self, call: lemurclaw_tools::ToolCall) -> lemurclaw_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: lemurclaw_tools::ToolCall,
        ) -> Result<Box<dyn lemurclaw_tools::ToolOutput>, lemurclaw_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: lemurclaw_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = lemurclaw_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        lemurclaw_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        lemurclaw_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
