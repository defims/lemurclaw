...
use lemurclaw_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use lemurclaw_protocol::openai_models::ToolMode;
use lemurclaw_protocol::protocol::MultiAgentVersion;
use lemurclaw_tools::ResponsesApiNamespace;
use lemurclaw_tools::ResponsesApiNamespaceTool;
use lemurclaw_tools::TOOL_SEARCH_TOOL_NAME;
use lemurclaw_tools::ToolCall as ExtensionToolCall;
use lemurclaw_tools::ToolEnvironmentMode;
use lemurclaw_tools::ToolExecutor;
...
use lemurclaw_tools::can_request_original_image_detail;
use lemurclaw_tools::collect_code_mode_exec_prompt_tool_definitions;
use lemurclaw_tools::collect_request_plugin_install_entries;
use lemurclaw_tools::default_namespace_description;
use lemurclaw_tools::request_user_input_available_modes;
use lemurclaw_tools::shell_command_backend_for_features;
use lemurclaw_tools::shell_type_for_model_and_features;
...
struct CoreToolPlanContext<'a> {
    ...
    mcp: &'a lemurclaw_mcp::McpBinding,
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && lemurclaw_code_mode::is_code_mode_nested_tool(spec.name())
    {
        lemurclaw_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && lemurclaw_code_mode::is_code_mode_nested_tool(&lemurclaw_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn build_code_mode_executors(
    ...
    if turn_context.model_info.use_responses_lite {
        let code_mode_tool_names =
            collect_code_mode_exec_prompt_tool_definitions(code_mode_nested_tool_specs.iter())
                ...
                .map(|tool| {
                    (
                        lemurclaw_code_mode::normalize_code_mode_identifier(&tool.name),
                        tool.tool_name,
                    )
                })
                .collect();
    ...
    let default_exec_yield_time_ms =
        default_exec_yield_time_override_ms(&turn_context.config.features)
            .unwrap_or(lemurclaw_code_mode::DEFAULT_EXEC_YIELD_TIME_MS);
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, lemurclaw_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| lemurclaw_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    let tool_mode = effective_tool_mode(turn_context);
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly) {
        reserved_tool_names.insert(ToolName::plain(lemurclaw_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(lemurclaw_code_mode::WAIT_TOOL_NAME));
    }
    if search_tool_enabled(turn_context)
        && planned_tools
            .runtimes()
...
fn compare_code_mode_tools(
    left: &lemurclaw_code_mode::ToolDefinition,
    right: &lemurclaw_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, lemurclaw_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &lemurclaw_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, lemurclaw_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
