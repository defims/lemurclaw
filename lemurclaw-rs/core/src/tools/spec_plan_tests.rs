...
fn zsh_fork_config_for_spec_plan_tests() -> lemurclaw_tools::ZshForkConfig {
    let placeholder_exe = lemurclaw_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(lemurclaw_tools::ResponsesApiNamespace {
    ...
    fn handle(&self, _call: ExtensionToolCall) -> lemurclaw_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(lemurclaw_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: lemurclaw_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    lemurclaw_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = lemurclaw_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(lemurclaw_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    lemurclaw_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        lemurclaw_code_mode::PUBLIC_TOOL_NAME,
        lemurclaw_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(lemurclaw_code_mode::PUBLIC_TOOL_NAME) else {
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if lemurclaw_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            lemurclaw_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
        ...
        turn.environments
            ...
            .push(TurnEnvironmentState::Ready(
                crate::session::turn_context::TurnEnvironment::new(
                    "remote".to_string(),
                    Arc::new(
                        lemurclaw_exec_server::Environment::create_for_tests(Some(
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(lemurclaw_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
