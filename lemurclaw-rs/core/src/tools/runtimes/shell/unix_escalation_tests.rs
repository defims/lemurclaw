...
use lemurclaw_execpolicy::PolicyParser;
use lemurclaw_execpolicy::RuleMatch;
use lemurclaw_hooks::Hooks;
use lemurclaw_hooks::HooksConfig;
use lemurclaw_network_proxy::PROXY_ACTIVE_ENV_KEY;
use lemurclaw_network_proxy::PROXY_ENV_KEYS;
...
use lemurclaw_protocol::protocol::GuardianCommandSource;
use lemurclaw_sandboxing::SandboxType;
use lemurclaw_sandboxing::policy_transforms::effective_permission_profile;
use lemurclaw_shell_escalation::EscalationExecution;
use lemurclaw_shell_escalation::EscalationPermissions;
use lemurclaw_shell_escalation::ExecResult;
use lemurclaw_shell_escalation::ResolvedPermissionProfile;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "LEMURCLAW_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(lemurclaw_execpolicy::Policy::empty())),
        ...
        stopwatch: lemurclaw_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = lemurclaw_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = lemurclaw_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let config_toml_path = turn_context
        .config
        .codex_home
        .join(lemurclaw_config::CONFIG_TOML_FILE);
    let hook_list = lemurclaw_hooks::list_hooks(HooksConfig {
    ...
    let expected_hook_command =
        lemurclaw_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(lemurclaw_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        lemurclaw_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        lemurclaw_shell_escalation::EscalationDecision::Escalate(
            lemurclaw_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, lemurclaw_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        lemurclaw_shell_escalation::EscalationDecision::Deny {
