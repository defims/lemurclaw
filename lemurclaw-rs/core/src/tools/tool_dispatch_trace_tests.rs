...
use lemurclaw_rollout_trace::ToolCallRequester;
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let replayed = lemurclaw_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
    ...
    let result = registry
        .dispatch_any_with_terminal_outcome(
            test_invocation_with_payload(
                ...
                turn,
                "incompatible-call",
                lemurclaw_tools::ToolName::plain("test_tool"),
                ToolCallSource::Direct,
                ToolPayload::Custom {
                    input: "{}".to_string(),
                },
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        lemurclaw_tools::ToolName::plain(tool_name),
        source,
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace =
        lemurclaw_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
