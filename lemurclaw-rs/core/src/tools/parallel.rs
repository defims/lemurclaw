    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: lemurclaw_protocol::models::FunctionCallOutputPayload {
                    body: lemurclaw_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    ...
                    trace_id = %lemurclaw_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    fn tool_call_timing_guard_ignores_code_mode_source() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            let call = ToolCall {
                tool_name: lemurclaw_tools::ToolName::plain("test_tool"),
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = lemurclaw_tools::ToolName::plain("test_tool");
    ...
    impl lemurclaw_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl lemurclaw_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: lemurclaw_extension_api::ToolFinishInput<'a>,
        ) -> lemurclaw_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let mut builder =
            lemurclaw_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = lemurclaw_tools::ToolName::plain("cleanup_tool");
