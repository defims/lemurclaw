...
use lemurclaw_protocol::error::SandboxErr;
...
use lemurclaw_protocol::protocol::ExecCommandOutputDeltaEvent;
use lemurclaw_protocol::protocol::ExecOutputStream;
use lemurclaw_sandboxing::SandboxCommand;
use lemurclaw_sandboxing::SandboxManager;
use lemurclaw_sandboxing::SandboxTransformRequest;
use lemurclaw_sandboxing::SandboxType;
use lemurclaw_sandboxing::SandboxablePreference;
use lemurclaw_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use lemurclaw_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use lemurclaw_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use lemurclaw_sandboxing::resolve_windows_elevated_filesystem_overrides;
use lemurclaw_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use lemurclaw_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use lemurclaw_sandboxing::windows_sandbox_uses_elevated_backend;
...
use lemurclaw_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: lemurclaw_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: lemurclaw_protocol::config_types::WindowsSandboxLevel,
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        lemurclaw_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
...
async fn exec_windows_sandbox(
    ...
    use lemurclaw_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use lemurclaw_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            lemurclaw_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                lemurclaw_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        lemurclaw_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                lemurclaw_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
