...
use lemurclaw_api::AgentIdentityTelemetry;
use lemurclaw_api::ApiError;
use lemurclaw_api::AuthProvider;
use lemurclaw_api::ChatCompletionsClient as ApiChatCompletionsClient;
use lemurclaw_api::ChatCompletionsOptions;
use lemurclaw_api::CompactClient as ApiCompactClient;
use lemurclaw_api::CompactionInput as ApiCompactionInput;
use lemurclaw_api::Compression;
use lemurclaw_api::MemoriesClient as ApiMemoriesClient;
use lemurclaw_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use lemurclaw_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use lemurclaw_api::Provider as ApiProvider;
use lemurclaw_api::RawMemory as ApiRawMemory;
use lemurclaw_api::RealtimeCallClient as ApiRealtimeCallClient;
use lemurclaw_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use lemurclaw_api::Reasoning;
use lemurclaw_api::ReasoningContext;
use lemurclaw_api::RequestTelemetry;
use lemurclaw_api::ReqwestTransport;
use lemurclaw_api::ResponseCreateWsRequest;
use lemurclaw_api::ResponsesApiRequest;
use lemurclaw_api::ResponsesClient as ApiResponsesClient;
use lemurclaw_api::ResponsesOptions as ApiResponsesOptions;
use lemurclaw_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use lemurclaw_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use lemurclaw_api::ResponsesWsRequest;
use lemurclaw_api::SharedAuthProvider;
use lemurclaw_api::SseTelemetry;
use lemurclaw_api::StreamOptions;
use lemurclaw_api::TransportError;
use lemurclaw_api::WebsocketTelemetry;
use lemurclaw_api::auth_header_telemetry;
use lemurclaw_api::build_session_headers;
use lemurclaw_api::create_text_param_for_request;
use lemurclaw_api::response_create_client_metadata;
...
use lemurclaw_login::RefreshTokenError;
use lemurclaw_login::UnauthorizedRecovery;
use lemurclaw_login::default_client::add_originator_header;
use lemurclaw_login::default_client::create_client_for_route;
use lemurclaw_otel::SessionTelemetry;
use lemurclaw_otel::current_span_w3c_trace_context;
use lemurclaw_protocol::auth::AuthMode;

use lemurclaw_protocol::ThreadId;
use lemurclaw_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use lemurclaw_protocol::config_types::Verbosity as VerbosityConfig;
...
use lemurclaw_protocol::protocol::InternalSessionSource;
use lemurclaw_protocol::protocol::SessionSource;
use lemurclaw_protocol::protocol::W3cTraceContext;
use lemurclaw_rollout_trace::CompactionTraceContext;
use lemurclaw_rollout_trace::InferenceTraceAttempt;
use lemurclaw_rollout_trace::InferenceTraceContext;
use lemurclaw_tools::create_tools_json_for_responses_api;
use lemurclaw_tools::create_tools_raw_json_for_responses_api;
...
use lemurclaw_feedback::FeedbackRequestTags;
use lemurclaw_feedback::emit_feedback_request_tags_with_auth_env;
use lemurclaw_login::auth::AgentIdentityAuthPolicy;
use lemurclaw_login::auth_env_telemetry::AuthEnvTelemetry;
use lemurclaw_login::auth_env_telemetry::collect_auth_env_telemetry;
use lemurclaw_model_provider::AgentIdentitySessionFallback;
use lemurclaw_model_provider::ProviderAuthScope;
use lemurclaw_model_provider::SharedModelProvider;
use lemurclaw_model_provider::create_model_provider;
#[cfg(test)]
use lemurclaw_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use lemurclaw_protocol::error::Result;
use lemurclaw_response_debug_context::extract_response_debug_context;
use lemurclaw_response_debug_context::extract_response_debug_context_from_api_error;
use lemurclaw_response_debug_context::telemetry_api_error_message;
use lemurclaw_response_debug_context::telemetry_transport_error_message;
...
impl ModelClient {
    ...
    fn build_responses_request(
        &self,
        provider: &lemurclaw_api::Provider,
        prompt: &Prompt,
        model_info: &ModelInfo,
        effort: Option<ReasoningEffortConfig>,
        summary: ReasoningSummaryConfig,
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: lemurclaw_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: lemurclaw_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &CodexResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                lemurclaw_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    /// `lemurclaw_api::sse::chat_completions`).
    async fn stream_chat_completions(
        ...
        Ok(crate::chat_completions::bridge_chat_completions_stream(api_stream))
    }
...
fn map_response_stream(
    ...
    let api_stream = lemurclaw_api::ResponseStream {
        rx_event,
