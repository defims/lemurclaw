...
use lemurclaw_config::CloudConfigBundleLoader;
...
use lemurclaw_config::ConstrainedWithSource;
use lemurclaw_config::FeatureRequirementsToml;
use lemurclaw_config::McpServerRequirement;
use lemurclaw_config::PluginRequirementsToml;
use lemurclaw_config::ProfileV2Name;
use lemurclaw_config::ResidencyRequirement;
use lemurclaw_config::SandboxModeRequirement;
use lemurclaw_config::Sourced;
use lemurclaw_config::ThreadConfigLoader;
use lemurclaw_config::config_toml::ConfigLockfileToml;
use lemurclaw_config::config_toml::ConfigToml;
use lemurclaw_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use lemurclaw_config::config_toml::ProjectConfig;
use lemurclaw_config::config_toml::RealtimeAudioConfig;
use lemurclaw_config::config_toml::RealtimeConfig;
use lemurclaw_config::config_toml::ThreadStoreToml;
use lemurclaw_config::config_toml::validate_model_providers;
use lemurclaw_config::loader::load_config_layers_state;
use lemurclaw_config::loader::project_trust_key;
use lemurclaw_config::permissions_toml::PermissionsToml;
use lemurclaw_config::sandbox_mode_requirement_for_permission_profile;
use lemurclaw_config::types::ApprovalsReviewer;
use lemurclaw_config::types::AuthCredentialsStoreMode;
use lemurclaw_config::types::AuthKeyringBackendKind;
use lemurclaw_config::types::History;
use lemurclaw_config::types::McpServerConfig;
use lemurclaw_config::types::McpServerDisabledReason;
...
use lemurclaw_config::types::OAuthCredentialsStoreMode;
use lemurclaw_config::types::ResumeCwdMode;
use lemurclaw_config::types::SessionPickerViewMode;
use lemurclaw_config::types::ToolSuggestConfig;
use lemurclaw_config::types::ToolSuggestDisabledTool;
use lemurclaw_config::types::ToolSuggestDiscoverable;
...
use lemurclaw_config::types::UriBasedFileOpener;
use lemurclaw_config::types::WindowsSandboxModeToml;
use lemurclaw_core_plugins::PluginLoadOutcome;
use lemurclaw_core_plugins::PluginsConfigInput;
use lemurclaw_exec_server::ExecutorFileSystem;
use lemurclaw_exec_server::LOCAL_FS;
use lemurclaw_features::CodeModeConfigToml;
use lemurclaw_features::CurrentTimeReminderConfigToml;
use lemurclaw_features::CurrentTimeReminderDeliveryMode;
use lemurclaw_features::CurrentTimeSource;
use lemurclaw_features::Feature;
use lemurclaw_features::FeatureConfigSource;
use lemurclaw_features::FeatureOverrides;
use lemurclaw_features::FeatureToml;
use lemurclaw_features::Features;
use lemurclaw_features::FeaturesToml;
use lemurclaw_features::MultiAgentV2ConfigToml;
use lemurclaw_features::NetworkProxyConfigToml;
use lemurclaw_features::TokenBudgetConfigToml;
...
use lemurclaw_install_context::InstallContext;
use lemurclaw_login::AuthManagerConfig;
use lemurclaw_login::AuthRouteConfig;
use lemurclaw_mcp::McpConfig;
use lemurclaw_mcp::McpPluginAttribution;
use lemurclaw_mcp::McpProtocolMode;
use lemurclaw_mcp::McpServerRegistration;
use lemurclaw_mcp::ResolvedMcpCatalog;
use lemurclaw_memories_read::memory_root;
use lemurclaw_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use lemurclaw_model_provider_info::ModelProviderInfo;
use lemurclaw_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use lemurclaw_model_provider_info::built_in_model_providers;
use lemurclaw_model_provider_info::merge_configured_model_providers;
use lemurclaw_models_manager::ModelsManagerConfig;
use lemurclaw_protocol::config_types::AltScreenMode;
...
use lemurclaw_protocol::config_types::ShellEnvironmentPolicy;
use lemurclaw_protocol::config_types::TrustLevel;
use lemurclaw_protocol::config_types::Verbosity;
use lemurclaw_protocol::config_types::WebSearchConfig;
...
pub use lemurclaw_thread_store::ExtraConfig;
use lemurclaw_utils_absolute_path::AbsolutePathBuf;
use lemurclaw_utils_absolute_path::AbsolutePathBufGuard;
...
pub use lemurclaw_config::ConfigLoadOptions;
pub use lemurclaw_config::Constrained;
pub use lemurclaw_config::ConstraintError;
pub use lemurclaw_config::ConstraintResult;
pub use lemurclaw_config::LoaderOverrides;
pub use lemurclaw_network_proxy::NetworkProxyAuditMetadata;
use lemurclaw_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use lemurclaw_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(lemurclaw_state::SQLITE_HOME_ENV).ok()?;
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<lemurclaw_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    pub sqlite: lemurclaw_state::SqliteConfig,
    ...
    pub otel: lemurclaw_config::types::OtelConfig,
}
...
impl ConfigBuilder {
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&lemurclaw_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = lemurclaw_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    lemurclaw_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(lemurclaw_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &lemurclaw_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &lemurclaw_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            connector_snapshot:
                lemurclaw_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn load_default_with_cli_overrides_for_codex_home(
        ...
        let cli_layer = lemurclaw_config::build_cli_overrides_layer(&cli_overrides);
        lemurclaw_config::merge_toml_values(&mut merged, &cli_layer);
...
pub fn set_default_oss_provider(codex_home: &Path, provider: &str) -> std::io::Result<()> {
    lemurclaw_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &lemurclaw_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            lemurclaw_protocol::permissions::FileSystemSandboxEntry {
                path: lemurclaw_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: lemurclaw_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            lemurclaw_protocol::permissions::FileSystemSandboxEntry {
                path: lemurclaw_protocol::permissions::FileSystemPath::Path { path },
                access: lemurclaw_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&lemurclaw_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(lemurclaw_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let config = Self {
            ...
            codex_home,
            sqlite: lemurclaw_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    lemurclaw_utils_home_dir::find_codex_home()
}
