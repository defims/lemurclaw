...
use lemurclaw_network_proxy::BlockedRequestObserver;
use lemurclaw_network_proxy::ConfigReloader;
use lemurclaw_network_proxy::ConfigReloaderFuture;
use lemurclaw_network_proxy::ConfigState;
use lemurclaw_network_proxy::NetworkDecision;
...
use lemurclaw_network_proxy::NetworkProxyConstraints;
use lemurclaw_network_proxy::NetworkProxyHandle;
use lemurclaw_network_proxy::NetworkProxyState;
use lemurclaw_network_proxy::build_config_state;
use lemurclaw_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use lemurclaw_network_proxy::managed_proxy_ports;
use lemurclaw_network_proxy::normalize_host;
use lemurclaw_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    lemurclaw_network_proxy::NetworkDomainPermission::Allow
                } else {
                    lemurclaw_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
