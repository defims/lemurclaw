...
use lemurclaw_config::ConfigLayerEntry;
use lemurclaw_config::ConfigLayerSource;
use lemurclaw_config::ConfigLayerStack;
use lemurclaw_config::McpServerCommandMatcher;
use lemurclaw_config::McpServerIdentity;
use lemurclaw_config::McpServerRequirement;
use lemurclaw_config::McpServerValueMatcher;
use lemurclaw_config::ProfileV2Name;
use lemurclaw_config::RequirementSource;
use lemurclaw_config::Sourced;
use lemurclaw_config::config_toml::AgentRoleToml;
use lemurclaw_config::config_toml::AgentsToml;
use lemurclaw_config::config_toml::AutoReviewToml;
use lemurclaw_config::config_toml::ConfigToml;
use lemurclaw_config::config_toml::ExperimentalRequestUserInput;
use lemurclaw_config::config_toml::ProjectConfig;
use lemurclaw_config::config_toml::RealtimeConfig;
use lemurclaw_config::config_toml::RealtimeToml;
use lemurclaw_config::config_toml::RealtimeTransport;
use lemurclaw_config::config_toml::RealtimeWsMode;
use lemurclaw_config::config_toml::RealtimeWsVersion;
use lemurclaw_config::config_toml::ToolsToml;
...
use lemurclaw_config::permissions_toml::NetworkDomainPermissionToml;
use lemurclaw_config::permissions_toml::NetworkDomainPermissionsToml;
use lemurclaw_config::permissions_toml::NetworkMitmActionToml;
use lemurclaw_config::permissions_toml::NetworkMitmHookToml;
use lemurclaw_config::permissions_toml::NetworkMitmToml;
...
use lemurclaw_config::permissions_toml::WorkspaceRootsToml;
use lemurclaw_config::types::AppToolApproval;
use lemurclaw_config::types::ApprovalsReviewer;
use lemurclaw_config::types::BundledSkillsConfig;
use lemurclaw_config::types::FeedbackConfigToml;
use lemurclaw_config::types::HistoryPersistence;
use lemurclaw_config::types::McpServerEnvVar;
use lemurclaw_config::types::McpServerOAuthConfig;
use lemurclaw_config::types::McpServerToolConfig;
use lemurclaw_config::types::McpServerTransportConfig;
use lemurclaw_config::types::MemoriesConfig;
use lemurclaw_config::types::MemoriesToml;
use lemurclaw_config::types::ModelAvailabilityNuxConfig;
use lemurclaw_config::types::Notice;
use lemurclaw_config::types::NotificationCondition;
use lemurclaw_config::types::NotificationMethod;
use lemurclaw_config::types::Notifications;
use lemurclaw_config::types::OtelConfigToml;
use lemurclaw_config::types::OtelExporterKind;
use lemurclaw_config::types::ResumeCwdMode;
use lemurclaw_config::types::SandboxWorkspaceWrite;
use lemurclaw_config::types::SessionPickerViewMode;
use lemurclaw_config::types::SkillsConfig;
use lemurclaw_config::types::ToolSuggestDisabledTool;
use lemurclaw_config::types::ToolSuggestDiscoverableType;
use lemurclaw_config::types::Tui;
use lemurclaw_config::types::TuiKeymap;
use lemurclaw_config::types::TuiNotificationSettings;
use lemurclaw_config::types::TuiPetAnchor;
use lemurclaw_config::types::WindowsSandboxModeToml;
use lemurclaw_config::types::WindowsToml;
...
use lemurclaw_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use lemurclaw_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use lemurclaw_model_provider_info::WireApi;
use lemurclaw_models_manager::bundled_models_response;
use lemurclaw_network_proxy::NetworkMode;
use lemurclaw_protocol::config_types::ModelProviderAuthInfo;
...
use lemurclaw_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use lemurclaw_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use lemurclaw_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use lemurclaw_protocol::models::ManagedFileSystemPermissions;
...
use lemurclaw_protocol::protocol::NetworkAccess;
...
use lemurclaw_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: lemurclaw_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
"#,
    ...
    assert_eq!(
        ...
        lemurclaw_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    assert_eq!(
        config
            .auth_route_config()
    ...
    assert_eq!(
        ...
        lemurclaw_core_plugins::remote::RemotePluginServiceConfig::new(
            config.chatgpt_base_url,
            lemurclaw_http_client::HttpClientFactory::new(
                lemurclaw_http_client::OutboundProxyPolicy::RespectSystemProxy,
            ),
        )
    );
...
"#,
    ...
    assert_eq!(
        resolve_bootstrap_auth_route_config(&configured, Some(&disabled))?
            .http_client_factory()
            .outbound_proxy_policy(),
        lemurclaw_http_client::OutboundProxyPolicy::ReqwestDefault
    );
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        lemurclaw_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".lemurclaw"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".lemurclaw".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".lemurclaw"), cwd.as_path()),
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".lemurclaw"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".lemurclaw"] {
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    lemurclaw_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            lemurclaw_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.lemurclaw", codex_home.path());
    ...
    let requirements_toml = lemurclaw_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = lemurclaw_config::ConfigRequirements {
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            lemurclaw_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, lemurclaw_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        lemurclaw_mcp::McpProtocolMode::V20260728
    );
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        lemurclaw_config::ConfigRequirementsToml {
...
"#,
    ...
    let config_layer_stack = lemurclaw_config::ConfigLayerStack::new(
        vec![lemurclaw_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
        ...
        Default::default(),
        lemurclaw_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".lemurclaw").join("agents");
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: lemurclaw_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = lemurclaw_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![lemurclaw_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = lemurclaw_config::RequirementSource::Unknown;
    ...
    let requirements = lemurclaw_config::ConfigRequirements {
        web_search_mode: lemurclaw_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        lemurclaw_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
"#,
    ...
    assert_eq!(
        ...
        Some(lemurclaw_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
