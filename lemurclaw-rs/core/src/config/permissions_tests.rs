...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let codex_home = temp_dir.path().join(".lemurclaw");
    ...
    let execve_wrapper = allowed_arg0_dir.join("lemurclaw-execve-wrapper");
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        config.unix_sockets,
        Some(lemurclaw_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        config.domains,
        Some(lemurclaw_network_proxy::NetworkDomainPermissions {
            entries: vec![lemurclaw_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: lemurclaw_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
