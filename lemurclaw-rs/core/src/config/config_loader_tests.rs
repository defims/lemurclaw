...
use lemurclaw_config::ConfigError;
...
use lemurclaw_config::ConfigRequirementsWithSources;
use lemurclaw_config::FilesystemDenyReadPattern;
use lemurclaw_config::LoaderOverrides;
use lemurclaw_config::RequirementSource;
use lemurclaw_config::RequirementsLayerEntry;
use lemurclaw_config::SessionThreadConfig;
use lemurclaw_config::StaticThreadConfigLoader;
use lemurclaw_config::ThreadConfigSource;
use lemurclaw_config::compose_requirements;
use lemurclaw_config::config_error_from_ignored_toml_fields;
use lemurclaw_config::config_error_from_toml;
use lemurclaw_config::config_error_from_typed_toml;
...
use lemurclaw_config::loader::load_requirements_toml;
...
use lemurclaw_protocol::config_types::EnvironmentVariablePattern;
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = lemurclaw_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        lemurclaw_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = lemurclaw_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = lemurclaw_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![lemurclaw_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(lemurclaw_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(lemurclaw_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            lemurclaw_config::SandboxModeRequirement::ReadOnly,
            lemurclaw_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    lemurclaw_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
mod requirements_exec_policy_tests {
    ...
    use lemurclaw_config::ConfigRequirementsWithSources;
    use lemurclaw_config::RequirementSource;
    use lemurclaw_config::RequirementsExecPolicyDecisionToml;
    use lemurclaw_config::RequirementsExecPolicyParseError;
    use lemurclaw_config::RequirementsExecPolicyPatternTokenToml;
    use lemurclaw_config::RequirementsExecPolicyPrefixRuleToml;
    use lemurclaw_config::RequirementsExecPolicyToml;
    use lemurclaw_execpolicy::Decision;
    use lemurclaw_execpolicy::Evaluation;
    use lemurclaw_execpolicy::RuleMatch;
    use lemurclaw_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
