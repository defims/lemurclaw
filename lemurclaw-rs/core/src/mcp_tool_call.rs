...
use lemurclaw_analytics::AppInvocation;
use lemurclaw_analytics::InvocationType;
use lemurclaw_analytics::build_track_events_context;
...
use lemurclaw_connectors::AppToolPolicy;
use lemurclaw_connectors::AppToolPolicyEvaluator;
use lemurclaw_connectors::AppToolPolicyInput;
use lemurclaw_features::Feature;
use lemurclaw_hooks::PermissionRequestDecision;
use lemurclaw_mcp::CODEX_APPS_MCP_SERVER_NAME;
use lemurclaw_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use lemurclaw_mcp::McpPermissionPromptAutoApproveContext;
use lemurclaw_mcp::PreparedMcpCall;
use lemurclaw_mcp::SandboxState;
use lemurclaw_mcp::auth_elicitation_completed_result;
use lemurclaw_mcp::build_auth_elicitation_plan;
use lemurclaw_mcp::mcp_permission_prompt_is_auto_approved;
use lemurclaw_protocol::approvals::ElicitationRequest;
use lemurclaw_protocol::items::McpToolCallError;
use lemurclaw_protocol::items::McpToolCallItem;
use lemurclaw_protocol::items::McpToolCallStatus;
use lemurclaw_protocol::items::TurnItem;
use lemurclaw_protocol::mcp::CallToolResult;
use lemurclaw_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use lemurclaw_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use lemurclaw_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use lemurclaw_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use lemurclaw_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use lemurclaw_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use lemurclaw_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use lemurclaw_protocol::request_user_input::RequestUserInputAnswer;
use lemurclaw_protocol::request_user_input::RequestUserInputArgs;
use lemurclaw_protocol::request_user_input::RequestUserInputQuestion;
use lemurclaw_protocol::request_user_input::RequestUserInputQuestionOption;
...
use lemurclaw_utils_output_truncation::TruncationPolicy;
use lemurclaw_utils_output_truncation::truncate_text;
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    let install_url = connector_id.map(|connector_id| {
        lemurclaw_connectors::metadata::connector_install_url(
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                lemurclaw_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == lemurclaw_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, lemurclaw_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
async fn maybe_request_mcp_tool_approval(
    ...
    hook_tool_name: &HookToolName,
    metadata: &McpToolApprovalMetadata,
    config: &lemurclaw_mcp::McpConfig,
    policy: McpToolApprovalPolicy,
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, lemurclaw_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
