...
pub(crate) async fn run_codex_thread_interactive(
    ...
    let (session, io) = Box::pin(Session::spawn(SessionSpawnArgs {
        ...
        parent_rollout_thread_trace: lemurclaw_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
        environment_selections: parent_ctx.environments.to_selections(),
        thread_extension_init: lemurclaw_extension_api::ExtensionDataInit::default(),
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                lemurclaw_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                lemurclaw_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                lemurclaw_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            lemurclaw_protocol::request_user_input::RequestUserInputAnswer {
...
) -> lemurclaw_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = lemurclaw_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(
                    approval_id,
                    lemurclaw_protocol::protocol::ReviewDecision::Abort,
                )
                .await;
            lemurclaw_protocol::protocol::ReviewDecision::Abort
        }
