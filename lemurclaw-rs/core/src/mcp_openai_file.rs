...
use lemurclaw_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use lemurclaw_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let file_name = resolved_path
        ...
        .unwrap_or("file")
        .to_string();
    let upload_auth = lemurclaw_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    fn set_primary_environment_cwd(turn_context: &mut TurnContext, cwd: &Path) {
        ...
        turn_context.permission_profile = lemurclaw_protocol::models::PermissionProfile::Disabled;
