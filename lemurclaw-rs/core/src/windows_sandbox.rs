...
pub fn sandbox_setup_is_complete(codex_home: &Path) -> bool {
    lemurclaw_windows_sandbox::sandbox_setup_is_complete(codex_home)
}
...
pub fn run_elevated_setup(
    ...
    let permissions =
        lemurclaw_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    lemurclaw_windows_sandbox::run_elevated_setup(
        lemurclaw_windows_sandbox::SandboxSetupRequest {
        ...
        lemurclaw_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
) -> std::io::Result<lemurclaw_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(lemurclaw_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(lemurclaw_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    lemurclaw_windows_sandbox::run_elevated_provisioning_setup(
        codex_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    lemurclaw_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        codex_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    lemurclaw_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = lemurclaw_otel::global() else {
        return;
    };
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = lemurclaw_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(lemurclaw_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if lemurclaw_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
