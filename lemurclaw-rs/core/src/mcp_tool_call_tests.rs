...
use lemurclaw_config::types::AppConfig;
use lemurclaw_config::types::AppToolConfig;
use lemurclaw_config::types::AppToolsConfig;
use lemurclaw_config::types::ApprovalsReviewer;
use lemurclaw_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> lemurclaw_mcp::McpConfig {
...
async fn codex_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        request.id,
        lemurclaw_protocol::mcp::RequestId::String("codex_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        lemurclaw_protocol::approvals::ElicitationRequest::Url { .. }
    ));
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_codex_dir = project_dir.path().join(".lemurclaw");
    ...
    ConfigEditsBuilder::new(&codex_home)
        .set_project_trust_level(
            project_dir.path(),
            lemurclaw_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
