...
use lemurclaw_analytics::CompactionTrigger;
use lemurclaw_analytics::HookRunFact;
use lemurclaw_analytics::build_track_events_context;
use lemurclaw_hooks::PermissionRequestDecision;
use lemurclaw_hooks::PermissionRequestOutcome;
use lemurclaw_hooks::PermissionRequestRequest;
use lemurclaw_hooks::PostToolUseOutcome;
use lemurclaw_hooks::PostToolUseRequest;
use lemurclaw_hooks::PreToolUseOutcome;
use lemurclaw_hooks::PreToolUseRequest;
use lemurclaw_hooks::SessionStartOutcome;
use lemurclaw_hooks::StartHookTarget;
use lemurclaw_hooks::StopHookTarget;
use lemurclaw_hooks::StopOutcome;
use lemurclaw_hooks::SubagentHookContext;
use lemurclaw_hooks::UserPromptSubmitOutcome;
use lemurclaw_hooks::UserPromptSubmitRequest;
use lemurclaw_otel::HOOK_RUN_DURATION_METRIC;
use lemurclaw_otel::HOOK_RUN_METRIC;
...
use lemurclaw_protocol::protocol::HookRunSummary;
use lemurclaw_protocol::protocol::HookSource;
use lemurclaw_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    lemurclaw_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = lemurclaw_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = lemurclaw_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = lemurclaw_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = lemurclaw_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(lemurclaw_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: lemurclaw_hooks::HookEvent::AfterAgent {
                event: lemurclaw_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            lemurclaw_hooks::HookResult::Success => continue,
            lemurclaw_hooks::HookResult::FailedContinue(error) => (error, false),
            lemurclaw_hooks::HookResult::FailedAbort(error) => (error, true),
        };
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (lemurclaw_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use lemurclaw_protocol::protocol::HookEventName;
    use lemurclaw_protocol::protocol::HookExecutionMode;
    use lemurclaw_protocol::protocol::HookHandlerType;
    use lemurclaw_protocol::protocol::HookRunStatus;
    use lemurclaw_protocol::protocol::HookScope;
    use lemurclaw_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    lemurclaw_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
