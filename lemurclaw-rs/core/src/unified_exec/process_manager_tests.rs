...
fn exec_env_policy_excludes_runtime_permission_profile() {
    ...
    assert_eq!(
        ...
        lemurclaw_exec_server::ExecEnvPolicy {
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: lemurclaw_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        lemurclaw_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = lemurclaw_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = lemurclaw_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: lemurclaw_exec_server::ExecEnvPolicy {
                inherit: lemurclaw_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: lemurclaw_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = lemurclaw_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_sandbox = Some(
        lemurclaw_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    ...
    assert_eq!(
        ...
        Some(lemurclaw_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: lemurclaw_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let lemurclaw_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let lemurclaw_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        lemurclaw_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            lemurclaw_exec_server::WriteStatus::Accepted,
