...
use lemurclaw_exec_server::ExecProcessEventReceiver;
use lemurclaw_exec_server::ExecProcessFuture;
...
use lemurclaw_exec_server::WriteResponse;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, lemurclaw_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, lemurclaw_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), lemurclaw_exec_server::ExecServerError> {
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = lemurclaw_exec_server::Environment::default_for_tests();
