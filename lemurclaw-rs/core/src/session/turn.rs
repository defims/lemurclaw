...
use lemurclaw_analytics::CompactionPhase;
use lemurclaw_analytics::CompactionReason;
use lemurclaw_analytics::InvocationType;
use lemurclaw_analytics::TurnResolvedConfigFact;
...
use lemurclaw_core_skills::injection::InjectedHostSkillPrompts;
use lemurclaw_extension_api::ExtensionData;
use lemurclaw_extension_api::TurnInputContext;
use lemurclaw_extension_api::TurnInputEnvironment;
use lemurclaw_features::Feature;
use lemurclaw_file_system::FindUpErrorPolicy;
use lemurclaw_file_system::find_nearest_ancestor_with_markers;
use lemurclaw_login::CodexAuth;
use lemurclaw_mcp::ToolInfo;
...
use lemurclaw_protocol::items::PlanItem;
...
use lemurclaw_protocol::protocol::RawResponseCompletedEvent;
use lemurclaw_protocol::protocol::ReasoningContentDeltaEvent;
use lemurclaw_protocol::protocol::ReasoningRawContentDeltaEvent;
use lemurclaw_protocol::protocol::SafetyBufferingEvent;
...
use lemurclaw_tools::DiscoverableTool;
use lemurclaw_tools::ToolName;
use lemurclaw_tools::filter_request_plugin_install_discoverable_tools_for_client;
use lemurclaw_utils_path_uri::PathUri;
use lemurclaw_utils_stream_parser::AssistantTextChunk;
use lemurclaw_utils_stream_parser::AssistantTextStreamParser;
use lemurclaw_utils_stream_parser::ProposedPlanSegment;
use lemurclaw_utils_stream_parser::extract_proposed_plan_text;
use lemurclaw_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "lemurclaw_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<lemurclaw_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &lemurclaw_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = lemurclaw_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
...
async fn capture_current_model_fallback_step_context(
    ...
    let uses_codex_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(lemurclaw_login::AuthManager::current_auth_uses_codex_backend);
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = lemurclaw_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<lemurclaw_extension_api::ExtensionData>,
...
pub(super) fn agent_message_text(item: &lemurclaw_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            lemurclaw_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: lemurclaw_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(lemurclaw_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &lemurclaw_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(lemurclaw_async_utils::CancelErr::Cancelled) => {
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![lemurclaw_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
