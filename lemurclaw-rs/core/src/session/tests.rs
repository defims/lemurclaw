...
use lemurclaw_config::ConfigLayerStackOrdering;
use lemurclaw_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use lemurclaw_config::LoaderOverrides;
use lemurclaw_config::NetworkConstraints;
use lemurclaw_config::NetworkDomainPermissionToml;
use lemurclaw_config::NetworkDomainPermissionsToml;
use lemurclaw_config::RequirementSource;
use lemurclaw_config::Sourced;
use lemurclaw_config::loader::project_trust_key;
use lemurclaw_config::types::McpServerConfig;
use lemurclaw_config::types::McpServerTransportConfig;
use lemurclaw_config::types::ToolSuggestDisabledTool;
use lemurclaw_core_skills::HostSkillsSnapshot;
use core_test_support::test_codex::local_selections;

use lemurclaw_features::Feature;
use lemurclaw_http_client::ClientRouteClass;
use lemurclaw_http_client::HttpClientFactory;
use lemurclaw_http_client::OutboundProxyPolicy;
use lemurclaw_http_client::RouteAwareClientPool;
use lemurclaw_login::CodexAuth;
use lemurclaw_login::auth::AgentIdentityAuthPolicy;
use lemurclaw_model_provider_info::ModelProviderInfo;
use lemurclaw_models_manager::bundled_models_response;
use lemurclaw_models_manager::model_info;
use lemurclaw_models_manager::test_support::construct_model_info_offline_for_tests;
use lemurclaw_models_manager::test_support::get_model_offline_for_tests;
use lemurclaw_protocol::AgentPath;
use lemurclaw_protocol::ResponseItemId;
use lemurclaw_protocol::SessionId;
use lemurclaw_protocol::ThreadId;
use lemurclaw_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use lemurclaw_protocol::config_types::ServiceTier;
use lemurclaw_protocol::config_types::TrustLevel;
use lemurclaw_protocol::exec_output::ExecToolCallOutput;
use lemurclaw_protocol::models::AgentMessageInputContent;
use lemurclaw_protocol::models::FileSystemPermissions;
use lemurclaw_protocol::models::FunctionCallOutputBody;
use lemurclaw_protocol::models::FunctionCallOutputContentItem;
use lemurclaw_protocol::models::FunctionCallOutputPayload;
use lemurclaw_protocol::models::ImageDetail;
use lemurclaw_protocol::models::PermissionProfile;
use lemurclaw_protocol::models::SandboxEnforcement;
use lemurclaw_protocol::openai_models::ModelServiceTier;
use lemurclaw_protocol::permissions::FileSystemAccessMode;
use lemurclaw_protocol::permissions::FileSystemPath;
use lemurclaw_protocol::permissions::FileSystemSandboxEntry;
use lemurclaw_protocol::permissions::FileSystemSandboxPolicy;
use lemurclaw_protocol::permissions::FileSystemSpecialPath;
use lemurclaw_protocol::protocol::NonSteerableTurnKind;
use lemurclaw_protocol::protocol::SandboxPolicy;
use lemurclaw_protocol::protocol::TurnEnvironmentSelections;
use lemurclaw_protocol::request_permissions::PermissionGrantScope;
use lemurclaw_protocol::request_permissions::RequestPermissionProfile;
use lemurclaw_utils_path_uri::PathUri;
use tracing::Span;
...
use lemurclaw_config::config_toml::ProjectConfig;
use lemurclaw_config::permissions_toml::FilesystemPermissionToml;
use lemurclaw_config::permissions_toml::FilesystemPermissionsToml;
use lemurclaw_config::permissions_toml::NetworkToml;
use lemurclaw_config::permissions_toml::PermissionProfileToml;
use lemurclaw_config::permissions_toml::PermissionsToml;
use lemurclaw_execpolicy::Decision;
use lemurclaw_execpolicy::NetworkRuleProtocol;
use lemurclaw_execpolicy::Policy;
use lemurclaw_network_proxy::NetworkProxyConfig;
use lemurclaw_otel::MetricsClient;
use lemurclaw_otel::MetricsConfig;
use lemurclaw_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use lemurclaw_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use lemurclaw_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use lemurclaw_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use lemurclaw_otel::TelemetryAuthMode;
...
use lemurclaw_protocol::items::HookPromptFragment;
use lemurclaw_protocol::items::build_hook_prompt_message;
use lemurclaw_protocol::models::BaseInstructions;
...
use lemurclaw_protocol::protocol::GranularApprovalConfig;
use lemurclaw_protocol::protocol::InitialHistory;
use lemurclaw_protocol::protocol::InterAgentCommunication;
use lemurclaw_protocol::protocol::MultiAgentVersion;
use lemurclaw_protocol::protocol::NetworkApprovalProtocol;
use lemurclaw_protocol::protocol::RateLimitSnapshot;
use lemurclaw_protocol::protocol::RateLimitWindow;
use lemurclaw_protocol::protocol::RealtimeAudioFrame;
use lemurclaw_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use lemurclaw_protocol::protocol::RealtimeVoice;
...
use lemurclaw_protocol::protocol::SessionMeta;
use lemurclaw_protocol::protocol::SessionMetaLine;
use lemurclaw_protocol::protocol::SkillScope;
use lemurclaw_protocol::protocol::Submission;
use lemurclaw_protocol::protocol::ThreadRolledBackEvent;
use lemurclaw_protocol::protocol::ThreadSettingsOverrides;
use lemurclaw_protocol::protocol::TokenCountEvent;
use lemurclaw_protocol::protocol::TokenUsage;
use lemurclaw_protocol::protocol::TokenUsageInfo;
use lemurclaw_protocol::protocol::TurnAbortedEvent;
use lemurclaw_protocol::protocol::TurnCompleteEvent;
use lemurclaw_protocol::protocol::TurnStartedEvent;
use lemurclaw_protocol::protocol::UserMessageEvent;
use lemurclaw_protocol::protocol::W3cTraceContext;
use lemurclaw_rmcp_client::ElicitationAction;
...
use lemurclaw_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<lemurclaw_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &lemurclaw_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(lemurclaw_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
            mcp_tools: Vec::new(),
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl lemurclaw_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: lemurclaw_extension_api::WorldStateContributionInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<
            'a,
            Vec<lemurclaw_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
    ...
    let mut builder = lemurclaw_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        lemurclaw_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        codex_home.join(lemurclaw_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<lemurclaw_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&lemurclaw_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: lemurclaw_hooks::StartHookTarget::SessionStart {
                source: lemurclaw_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn lemurclaw_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { lemurclaw_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<()> for ProbeToolRuntime {
        ...
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
                #[allow(deprecated)]
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> lemurclaw_sandboxing::SandboxablePreference {
            lemurclaw_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(lemurclaw_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: lemurclaw_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = lemurclaw_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    assert_eq!(
        config
            ...
            .map(lemurclaw_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    ...
    let config_toml_path = codex_home.join(CONFIG_TOML_FILE);
    let user_config: lemurclaw_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = lemurclaw_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = lemurclaw_hooks::list_hooks(lemurclaw_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..lemurclaw_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        lemurclaw_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: lemurclaw_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: lemurclaw_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: lemurclaw_config::MatcherGroup {
                matcher: None,
                hooks: vec![lemurclaw_config::HookHandlerConfig::Command {
        ...
        let identity = lemurclaw_config::TomlValue::try_from(identity)?;
        lemurclaw_config::version_for_toml(&identity)
    };
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    let requirement = serde_json::from_value::<lemurclaw_config::McpServerRequirement>(json!({
        "identity": { "url": "https://example.com/mcp" }
    }))
    .expect("valid managed MCP requirement");
    let plugin_requirements = std::collections::BTreeMap::from([(
        "example-plugin".to_string(),
        lemurclaw_config::PluginRequirementsToml {
    ...
    next_config.mcp_servers = lemurclaw_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = lemurclaw_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: lemurclaw_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl lemurclaw_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a lemurclaw_extension_api::ExtensionData,
            thread_store: &'a lemurclaw_extension_api::ExtensionData,
            turn_store: &'a lemurclaw_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl lemurclaw_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: lemurclaw_extension_api::TurnStartInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl lemurclaw_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: lemurclaw_extension_api::TurnErrorInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl lemurclaw_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &lemurclaw_extension_api::ExtensionData,
            thread_store: &lemurclaw_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: lemurclaw_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            lemurclaw_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        lemurclaw_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: lemurclaw_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            lemurclaw_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            lemurclaw_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            lemurclaw_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(lemurclaw_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(lemurclaw_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = lemurclaw_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        lemurclaw_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        codex_home.path().join(lemurclaw_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&lemurclaw_exec_server::LOCAL_FS));
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(lemurclaw_code_mode::InProcessCodeModeSessionProvider),
        Arc::new(lemurclaw_extension_api::ExtensionRegistryBuilder::new().build()),
        lemurclaw_extension_api::ExtensionDataInit::default(),
        /*supports_openai_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(lemurclaw_thread_store::LocalThreadStore::new(
            lemurclaw_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        lemurclaw_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        lemurclaw_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(lemurclaw_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    let services = SessionServices {
        ...
        rollout_thread_trace: lemurclaw_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(lemurclaw_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: lemurclaw_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: lemurclaw_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: lemurclaw_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(lemurclaw_thread_store::LocalThreadStore::new(
            lemurclaw_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(lemurclaw_thread_store::LocalThreadStore::new(
            lemurclaw_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                lemurclaw_state::StateRuntime::init(
                    config.sqlite.clone(),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            lemurclaw_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(lemurclaw_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = lemurclaw_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(lemurclaw_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    lemurclaw_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: lemurclaw_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(lemurclaw_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: lemurclaw_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(local_selections(config.cwd.clone())),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(lemurclaw_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(lemurclaw_protocol::config_types::CollaborationMode {
                    mode: lemurclaw_protocol::config_types::ModeKind::Default,
                    settings: lemurclaw_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        lemurclaw_config::types::ApprovalsReviewer::AutoReview
    );
    assert!(
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = lemurclaw_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        lemurclaw_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = lemurclaw_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(lemurclaw_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn lemurclaw_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        lemurclaw_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: lemurclaw_extension_api::ThreadStopInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: lemurclaw_extension_api::ThreadStopInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl lemurclaw_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: lemurclaw_extension_api::TurnAbortInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            lemurclaw_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = lemurclaw_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !lemurclaw_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        lemurclaw_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        lemurclaw_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = lemurclaw_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: lemurclaw_rmcp_client::Elicitation::Mcp(
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    ...
    lemurclaw_login::login_with_api_key(
        codex_home.path(),
        "new-api-key",
        lemurclaw_login::AuthCredentialsStoreMode::File,
        lemurclaw_login::AuthKeyringBackendKind::default(),
    )
...
impl lemurclaw_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: lemurclaw_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<lemurclaw_exec_server::NoiseRendezvousConnectBundle, lemurclaw_exec_server::ExecServerError>,
    > {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root =
        |environment_id: &str, path: &str| lemurclaw_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: lemurclaw_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let lemurclaw_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        registration
            .complete(Ok(lemurclaw_exec_server::EnvironmentReadyInfo {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(lemurclaw_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(lemurclaw_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl lemurclaw_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a lemurclaw_extension_api::ExtensionData,
        thread_store: &'a lemurclaw_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<lemurclaw_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    lemurclaw_extension_api::PromptFragment::developer_policy(
...
-> Arc<lemurclaw_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = lemurclaw_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl lemurclaw_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: lemurclaw_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<lemurclaw_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<lemurclaw_thread_store::InMemoryThreadStore> {
    let store = Arc::new(lemurclaw_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn lemurclaw_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &lemurclaw_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: lemurclaw_extension_api::ThreadIdleInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = lemurclaw_protocol::user_input::TextElement::new(
        lemurclaw_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: lemurclaw_extension_api::ThreadIdleInput<'a>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(lemurclaw_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use lemurclaw_protocol::protocol::AskForApproval;
    use lemurclaw_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: lemurclaw_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: lemurclaw_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let expected_source_path = lemurclaw_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
