...
use super::*;
use lemurclaw_exec_server::ExecutorCapabilityDiscoveryCache;
use lemurclaw_exec_server::ExecutorCapabilityDiscoverySnapshot;
use lemurclaw_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use lemurclaw_exec_server::ResolvedSelectedCapabilityRoot;
use lemurclaw_mcp::CODEX_APPS_MCP_SERVER_NAME;
use lemurclaw_mcp::ElicitationReviewRequest;
use lemurclaw_mcp::ElicitationReviewer;
use lemurclaw_mcp::ElicitationReviewerHandle;
use lemurclaw_protocol::capabilities::SelectedCapabilityRoot;
use lemurclaw_protocol::config_types::ApprovalsReviewer;
use lemurclaw_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use lemurclaw_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use lemurclaw_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use lemurclaw_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use lemurclaw_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use lemurclaw_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use lemurclaw_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use lemurclaw_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use lemurclaw_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        lemurclaw_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn hard_refresh_latest_codex_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<lemurclaw_mcp::ToolInfo>> {
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        Arc::new(lemurclaw_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> lemurclaw_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                lemurclaw_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: lemurclaw_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                lemurclaw_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                lemurclaw_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    match approval_policy {
        AskForApproval::Never => {
            if lemurclaw_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                &mcp_config.permission_profile,
                lemurclaw_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let lemurclaw_protocol::approvals::ElicitationRequest::Form {
