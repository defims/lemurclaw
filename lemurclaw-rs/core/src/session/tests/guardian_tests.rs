...
use lemurclaw_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                lemurclaw_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = lemurclaw_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config
        .permissions
        .set_permission_profile(lemurclaw_protocol::models::PermissionProfile::Disabled)
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        lemurclaw_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: lemurclaw_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    let thread_store = Arc::new(lemurclaw_thread_store::LocalThreadStore::new(
        lemurclaw_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(lemurclaw_code_mode::InProcessCodeModeSessionProvider),
        extensions: lemurclaw_extension_api::empty_extension_registry(),
        ...
        windows_sandbox_proxy_settings_mode:
            lemurclaw_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
    })
    .await
