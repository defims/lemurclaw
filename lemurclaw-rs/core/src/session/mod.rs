...
use lemurclaw_analytics::SubAgentThreadStartedInput;
use lemurclaw_analytics::TurnCodexErrorFact;
use lemurclaw_async_utils::OrCancelExt;
use lemurclaw_connectors::connector_runtime_context_key;
use lemurclaw_core_skills::injection::HostSkillsCatalogInWorldState;
use lemurclaw_exec_server::Environment;
use lemurclaw_exec_server::EnvironmentManager;
use lemurclaw_execpolicy::prefix_rule_migration;
use lemurclaw_extension_api::ExtensionDataInit;
use lemurclaw_extension_api::LoadedUserInstructions;
use lemurclaw_extension_api::PromptFragment;
use lemurclaw_extension_api::PromptSlot;
use lemurclaw_extension_api::TurnContextContributionInput;
use lemurclaw_features::FEATURES;
use lemurclaw_features::Feature;
use lemurclaw_features::unstable_features_warning_event;
use lemurclaw_hooks::Hooks;
use lemurclaw_hooks::HooksConfig;
use lemurclaw_login::AuthManager;
use lemurclaw_login::CodexAuth;
use lemurclaw_login::auth_env_telemetry::collect_auth_env_telemetry;
use lemurclaw_mcp::McpResourceClient;
use lemurclaw_mcp::McpRuntime;
use lemurclaw_mcp::McpRuntimeContext;
use lemurclaw_mcp::McpRuntimeInput;
use lemurclaw_models_manager::manager::RefreshStrategy;
use lemurclaw_models_manager::manager::SharedModelsManager;
use lemurclaw_network_proxy::NetworkProxy;
use lemurclaw_network_proxy::NetworkProxyAuditMetadata;
use lemurclaw_network_proxy::normalize_host;
use lemurclaw_otel::current_span_trace_id;
use lemurclaw_otel::current_span_w3c_trace_context;
use lemurclaw_otel::set_parent_from_w3c_trace_context;
use lemurclaw_protocol::SessionId;
use lemurclaw_protocol::ThreadId;
use lemurclaw_protocol::approvals::ElicitationRequest;
use lemurclaw_protocol::approvals::ElicitationRequestEvent;
use lemurclaw_protocol::approvals::ExecPolicyAmendment;
use lemurclaw_protocol::approvals::NetworkPolicyAmendment;
use lemurclaw_protocol::approvals::NetworkPolicyRuleAction;
use lemurclaw_protocol::config_types::ApprovalsReviewer;
use lemurclaw_protocol::config_types::AutoCompactTokenLimitScope;
...
use lemurclaw_protocol::config_types::WebSearchMode;
use lemurclaw_protocol::dynamic_tools::DynamicToolResponse;
use lemurclaw_protocol::dynamic_tools::DynamicToolSpec;
use lemurclaw_protocol::items::EnteredReviewModeItem;
use lemurclaw_protocol::items::TurnItem;
use lemurclaw_protocol::items::UserMessageItem;
use lemurclaw_protocol::models::ActivePermissionProfile;
use lemurclaw_protocol::models::AdditionalPermissionProfile;
...
use lemurclaw_protocol::models::format_allow_prefixes;
use lemurclaw_protocol::openai_models::ModelInfo;
use lemurclaw_protocol::openai_models::ModelPreset;
use lemurclaw_protocol::permissions::FileSystemSandboxPolicy;
use lemurclaw_protocol::permissions::NetworkSandboxPolicy;
use lemurclaw_protocol::protocol::AdditionalContextEntry;
use lemurclaw_protocol::protocol::FileChange;
use lemurclaw_protocol::protocol::HasLegacyEvent;
...
use lemurclaw_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use lemurclaw_protocol::protocol::MultiAgentVersion;
use lemurclaw_protocol::protocol::RawResponseItemEvent;
...
use lemurclaw_protocol::protocol::ThreadHistoryMode;
use lemurclaw_protocol::protocol::ThreadSource;
use lemurclaw_protocol::protocol::TurnAbortReason;
use lemurclaw_protocol::protocol::TurnContextItem;
use lemurclaw_protocol::protocol::TurnContextNetworkItem;
...
use lemurclaw_protocol::protocol::WorldStateItem;
...
use lemurclaw_protocol::request_permissions::RequestPermissionsArgs;
use lemurclaw_protocol::request_permissions::RequestPermissionsEvent;
use lemurclaw_protocol::request_permissions::RequestPermissionsResponse;
use lemurclaw_protocol::request_user_input::RequestUserInputArgs;
use lemurclaw_protocol::request_user_input::RequestUserInputResponse;
use lemurclaw_rmcp_client::ElicitationResponse;
use lemurclaw_rollout::state_db;
use lemurclaw_rollout_trace::AgentResultTracePayload;
use lemurclaw_rollout_trace::ThreadStartedTraceMetadata;
use lemurclaw_rollout_trace::ThreadTraceContext;
use lemurclaw_sandboxing::policy_transforms::intersect_permission_profiles;
use lemurclaw_shell_command::parse_command::parse_command;
use lemurclaw_terminal_detection::user_agent;
use lemurclaw_thread_store::CreateThreadParams;
use lemurclaw_thread_store::LiveThread;
use lemurclaw_thread_store::LiveThreadInitGuard;
use lemurclaw_thread_store::LocalThreadStore;
use lemurclaw_thread_store::ReadThreadParams;
use lemurclaw_thread_store::ResumeThreadParams;
use lemurclaw_thread_store::ThreadPersistenceMetadata;
...
use lemurclaw_config::CONFIG_TOML_FILE;
use lemurclaw_config::ConfigLayerSource;
...
use lemurclaw_protocol::error::CodexErrorDetails;
use lemurclaw_protocol::error::Result as CodexResult;
#[cfg(test)]
use lemurclaw_protocol::exec_output::StreamOutput;
...
use lemurclaw_core_plugins::RecommendedPluginCandidatesInput;
use lemurclaw_git_utils::get_git_repo_root;
use lemurclaw_mcp::McpConfig;
use lemurclaw_mcp::effective_mcp_servers;
use lemurclaw_otel::SessionTelemetry;
use lemurclaw_otel::THREAD_STARTED_METRIC;
...
use lemurclaw_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use lemurclaw_protocol::config_types::WindowsSandboxLevel;
use lemurclaw_protocol::models::LocalImagePreparation;
...
use lemurclaw_protocol::protocol::ModelRerouteEvent;
use lemurclaw_protocol::protocol::ModelRerouteReason;
use lemurclaw_protocol::protocol::ModelVerification;
use lemurclaw_protocol::protocol::ModelVerificationEvent;
use lemurclaw_protocol::protocol::NetworkApprovalContext;
...
use lemurclaw_protocol::protocol::RequestUserInputEvent;
...
use lemurclaw_protocol::protocol::SessionNetworkProxyRuntime;
use lemurclaw_protocol::protocol::StreamErrorEvent;
use lemurclaw_protocol::protocol::Submission;
use lemurclaw_protocol::protocol::ThreadMemoryMode;
...
use lemurclaw_protocol::protocol::TurnModerationMetadataEvent;
use lemurclaw_protocol::protocol::WarningEvent;
use lemurclaw_protocol::user_input::UserInput;
use lemurclaw_tools::UnifiedExecShellMode;
use lemurclaw_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use lemurclaw_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn lemurclaw_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<lemurclaw_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        lemurclaw_sandboxing::WindowsSandboxProxySettingsMode,
}
...
impl Session {
    ...
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if lemurclaw_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            lemurclaw_models_manager::manager::RefreshStrategy::Offline
        } else {
            lemurclaw_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &lemurclaw_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn lemurclaw_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn lemurclaw_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn user_instructions(&self) -> Option<lemurclaw_extension_api::UserInstructions> {
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &lemurclaw_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| lemurclaw_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => lemurclaw_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                GuardianReviewOptions {
                    ...
                    approval_request_source:
                        lemurclaw_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn capture_step_context_with_required_mcp_servers(
        ...
        let executor_capability_discovery = self
            ...
            .await;
        let extension_data = lemurclaw_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(lemurclaw_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<lemurclaw_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
