...
pub use lemurclaw_connectors::AppBranding;
pub use lemurclaw_connectors::AppInfo;
pub use lemurclaw_connectors::AppMetadata;
use lemurclaw_connectors::ConnectorDirectoryCacheContext;
use lemurclaw_connectors::ConnectorDirectoryCacheKey;
use lemurclaw_connectors::app_is_enabled;
use lemurclaw_connectors::apps_config_from_layer_stack;
...
use lemurclaw_mcp::ToolPluginProvenance;
use lemurclaw_mcp::effective_mcp_servers;
use lemurclaw_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = lemurclaw_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        lemurclaw_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let codex_apps_auth_manager =
        lemurclaw_mcp::host_owned_codex_apps_enabled(&mcp_config, auth.as_ref())
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + lemurclaw_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    lemurclaw_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(lemurclaw_connectors::accessible::AccessibleConnectorTool {
    ...
    lemurclaw_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &lemurclaw_config::ConfigLayerStack,
