...
use lemurclaw_analytics::GuardianReviewDecision;
use lemurclaw_analytics::GuardianReviewFailureReason;
use lemurclaw_analytics::GuardianReviewTerminalStatus;
use lemurclaw_analytics::GuardianReviewTrackContext;
use lemurclaw_analytics::GuardianReviewedAction;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &lemurclaw_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        lemurclaw_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == lemurclaw_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == lemurclaw_protocol::openai_models::ReasoningEffort::Low),
