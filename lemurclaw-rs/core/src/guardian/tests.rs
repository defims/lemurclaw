...
use lemurclaw_analytics::GuardianApprovalRequestSource;
...
use lemurclaw_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
use lemurclaw_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
use lemurclaw_model_provider_info::ModelProviderInfo;
use lemurclaw_model_provider_info::OPENAI_PROVIDER_ID;
use lemurclaw_models_manager::manager::StaticModelsManager;
...
use lemurclaw_protocol::protocol::GuardianRiskLevel;
use lemurclaw_protocol::protocol::GuardianUserAuthorization;
...
impl lemurclaw_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: lemurclaw_extension_api::ThreadStartInput<'a, Config>,
...
impl lemurclaw_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a lemurclaw_extension_api::ExtensionData,
        thread_store: &'a lemurclaw_extension_api::ExtensionData,
    ) -> lemurclaw_extension_api::ExtensionFuture<'a, Vec<lemurclaw_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![lemurclaw_extension_api::PromptFragment::developer_policy(
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: lemurclaw_protocol::models::FunctionCallOutputPayload::from_text(
...
fn guardian_prompt_text(items: &[lemurclaw_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            lemurclaw_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: lemurclaw_protocol::permissions::FileSystemSpecialPath::Root,
                },
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    lemurclaw_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = lemurclaw_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(lemurclaw_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            lemurclaw_protocol::user_input::UserInput::Text { text, .. } => text,
            lemurclaw_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(lemurclaw_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(lemurclaw_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
