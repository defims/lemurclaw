...
use lemurclaw_config::types::OtelExporterKind as Kind;
use lemurclaw_config::types::OtelHttpProtocol as Protocol;
use lemurclaw_features::Feature;
use lemurclaw_login::default_client::originator;
use lemurclaw_otel::OtelExporter;
use lemurclaw_otel::OtelHttpProtocol;
use lemurclaw_otel::OtelProvider;
use lemurclaw_otel::OtelSettings;
use lemurclaw_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = lemurclaw_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = lemurclaw_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
