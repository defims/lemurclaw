...
impl lemurclaw_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: lemurclaw_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> lemurclaw_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<lemurclaw_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> lemurclaw_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<lemurclaw_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> lemurclaw_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: lemurclaw_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl lemurclaw_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: lemurclaw_extension_api::McpServerContributionContext<'a, Config>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, Vec<lemurclaw_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let mut extensions = lemurclaw_extension_api::ExtensionRegistryBuilder::new();
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl lemurclaw_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl lemurclaw_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        fn contribute<'a>(
            &'a self,
            context: lemurclaw_extension_api::McpServerContributionContext<'a, Config>,
        ) -> lemurclaw_extension_api::ExtensionFuture<'a, Vec<lemurclaw_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = lemurclaw_mcp::codex_apps_mcp_server_config(
                ...
                vec![lemurclaw_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = lemurclaw_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &lemurclaw_mcp::McpConfig| {
        lemurclaw_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let codex_apps_server = lemurclaw_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(lemurclaw_mcp::CODEX_APPS_MCP_SERVER_NAME)
        .expect("Codex Apps server should be configured");
    let codex_apps_headers = match codex_apps_server.transport {
        lemurclaw_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        lemurclaw_config::McpServerTransportConfig::Stdio { .. } => {
