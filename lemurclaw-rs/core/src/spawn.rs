...
pub const CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "LEMURCLAW_SANDBOX_NETWORK_DISABLED";
...
pub const CODEX_SANDBOX_ENV_VAR: &str = "LEMURCLAW_SANDBOX";
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                lemurclaw_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                lemurclaw_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
