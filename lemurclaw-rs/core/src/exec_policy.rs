...
use lemurclaw_execpolicy::AmendError;
use lemurclaw_execpolicy::Decision;
use lemurclaw_execpolicy::Error as ExecPolicyRuleError;
...
use lemurclaw_execpolicy::blocking_append_allow_prefix_rule;
use lemurclaw_execpolicy::blocking_append_network_rule;
...
use lemurclaw_shell_command::is_dangerous_command::DangerousCommandMatch;
use lemurclaw_shell_command::is_dangerous_command::dangerous_command_match;
use lemurclaw_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(lemurclaw_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: lemurclaw_execpolicy::Error,
    },
}
...
fn exec_policy_message_for_display(source: &lemurclaw_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            lemurclaw_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            lemurclaw_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            lemurclaw_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
