...
use anyhow::ensure;
use lemurclaw_arg0::Arg0PathEntryGuard;
use lemurclaw_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use lemurclaw_core::test_support::TestCodexResponsesRequestKind;
pub use lemurclaw_core::test_support::responses_metadata;
use lemurclaw_utils_absolute_path::AbsolutePathBuf;
pub use lemurclaw_utils_absolute_path::test_support::PathBufExt;
pub use lemurclaw_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    lemurclaw_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    lemurclaw_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(lemurclaw_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = lemurclaw_utils_cargo_bin::repo_root()
        .ok()
...
/// avoids clobbering a developer’s real `~/.lemurclaw`.
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        codex_linux_sandbox_exe: Some(
            find_codex_linux_sandbox_exe().expect("should find binary for lemurclaw-linux-sandbox"),
        ),
...
pub fn find_codex_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    lemurclaw_utils_cargo_bin::cargo_bin("lemurclaw-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &CodexThread,
    predicate: F,
) -> lemurclaw_protocol::protocol::EventMsg
where
    F: FnMut(&lemurclaw_protocol::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&lemurclaw_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    lemurclaw_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    lemurclaw_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    lemurclaw_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    lemurclaw_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    lemurclaw_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
    ...
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_codex_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("lemurclaw-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
