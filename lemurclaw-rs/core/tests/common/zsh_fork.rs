...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = lemurclaw_utils_cargo_bin::cargo_bin("lemurclaw-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `lemurclaw-execve-wrapper` binary");
        return Ok(None);
    };
