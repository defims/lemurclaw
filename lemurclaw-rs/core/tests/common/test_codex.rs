...
use lemurclaw_core::TimeProvider;
use lemurclaw_core::config::Config;
use lemurclaw_core::resolve_installation_id;
use lemurclaw_core::shell::Shell;
use lemurclaw_core::shell::get_shell_by_model_provided_path;
use lemurclaw_core::thread_store_from_config;
use lemurclaw_exec_server::CreateDirectoryOptions;
use lemurclaw_exec_server::ExecutorFileSystem;
use lemurclaw_exec_server::RemoveOptions;
use lemurclaw_extension_api::ExtensionRegistry;
use lemurclaw_extension_api::LoadUserInstructionsFuture;
use lemurclaw_extension_api::UserInstructionsProvider;
use lemurclaw_extension_api::empty_extension_registry;
use lemurclaw_features::Feature;
use lemurclaw_home::CodexHomeUserInstructionsProvider;
...
use lemurclaw_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "LEMURCLAW_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: lemurclaw_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: lemurclaw_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            lemurclaw_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &lemurclaw_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                lemurclaw_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    async fn build_with_home_and_base_url(
        ...
        let codex_linux_sandbox_exe = Some(
            crate::find_codex_linux_sandbox_exe()
                .context("should find binary for lemurclaw-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let codex_linux_sandbox_exe = None;
        let local_runtime_paths = lemurclaw_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            codex_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            lemurclaw_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            lemurclaw_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<lemurclaw_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        ...
        let auth_manager = lemurclaw_core::test_support::auth_manager_from_auth(auth.clone());
        let thread_manager = ThreadManager::new(
            &config,
            auth_manager.clone(),
            lemurclaw_core::build_models_manager(&config, auth_manager),
            lemurclaw_core::CodexAppsToolsCache::default(),
            SessionSource::Exec,
            ...
            thread_store,
            lemurclaw_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| lemurclaw_utils_cargo_bin::cargo_bin("codex-code-mode-host").ok());
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            lemurclaw_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
                &config,
            )
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                let auth_manager = lemurclaw_core::test_support::auth_manager_from_auth(auth);
                Box::pin(
                    lemurclaw_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    lemurclaw_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = lemurclaw_utils_cargo_bin::cargo_bin("codex") {
            config.codex_self_exe = Some(path);
        } else if let Ok(path) = lemurclaw_utils_cargo_bin::cargo_bin("codex-exec") {
            // `codex-exec` also supports `--lemurclaw-run-as-apply-patch`, so use it
