...
use lemurclaw_extension_api::McpServerContributor;
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, lemurclaw_config::types::McpServerConfig>::deserialize)
        .transpose()?
        ...
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == CODEX_APPS_MCP_SERVER_NAME
                    && matches!(update.status, lemurclaw_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
