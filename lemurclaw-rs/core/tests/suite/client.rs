...
use lemurclaw_core::ModelClient;
use lemurclaw_core::NewThread;
use lemurclaw_core::Prompt;
use lemurclaw_core::ResponseEvent;
...
use lemurclaw_login::AuthKeyringBackendKind;
...
use lemurclaw_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use lemurclaw_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use lemurclaw_protocol::models::LocalShellAction;
use lemurclaw_protocol::models::LocalShellExecAction;
use lemurclaw_protocol::models::LocalShellStatus;
use lemurclaw_protocol::models::MessagePhase;
use lemurclaw_protocol::models::ReasoningItemContent;
use lemurclaw_protocol::models::ReasoningItemReasoningSummary;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> lemurclaw_core::CodexResponsesMetadata {
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: lemurclaw_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = lemurclaw_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = lemurclaw_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![lemurclaw_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = lemurclaw_protocol::models::ResponseItem::Message {
        id: None,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let model = lemurclaw_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        lemurclaw_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &lemurclaw_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = CodexAuth::from_auth_storage(
        ...
        &lemurclaw_login::test_support::transport_default_auth_route_config(),
    )
    .await
    ...
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(lemurclaw_core::test_support::EmptyUserInstructionsProvider),
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        lemurclaw_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
