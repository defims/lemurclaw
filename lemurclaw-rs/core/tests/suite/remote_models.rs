...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let manager = lemurclaw_core::test_support::models_manager_with_provider(
        codex_home.path().to_path_buf(),
        lemurclaw_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
...
fn bundled_default_model_slug() -> String {
    lemurclaw_core::test_support::all_model_presets()
        .iter()
