...
use anyhow::Result;
use lemurclaw_core_plugins::store::PluginStore;
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context() -> Result<()> {
    ...
    codex
        .submit(Op::UserInput {
            items: vec![lemurclaw_protocol::user_input::UserInput::Text {
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins() -> Result<()> {
    ...
    codex
        .submit(Op::UserInput {
            items: vec![lemurclaw_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    assert_eq!(
        ...
        serde_json::json!(lemurclaw_login::default_client::originator().value)
    );
