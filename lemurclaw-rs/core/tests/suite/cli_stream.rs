...
fn repo_root() -> std::path::PathBuf {
    lemurclaw_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = lemurclaw_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("LEMURCLAW_HOME", home.path())
        .env(CODEX_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("LEMURCLAW_AUTHAPI_BASE_URL", server.uri())
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = lemurclaw_utils_pty::process_group::kill_process_group(self.0);
        }
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = lemurclaw_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("LEMURCLAW_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");
