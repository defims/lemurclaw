...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != lemurclaw_code_mode::PUBLIC_TOOL_NAME
                && name != lemurclaw_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            lemurclaw_code_mode::PUBLIC_TOOL_NAME.to_string(),
            lemurclaw_code_mode::WAIT_TOOL_NAME.to_string(),
    ...
    assert!(
        tool_names(&unsupported_response.body)
            .iter()
            .any(|name| name == lemurclaw_code_mode::PUBLIC_TOOL_NAME)
    );
    assert_eq!(
