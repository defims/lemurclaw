...
use lemurclaw_protocol::items::parse_hook_prompt_fragment;
...
fn trust_plugin_hooks(config: &mut Config, plugin_hook_sources: Vec<PluginHookSource>) {
    ...
    let listed = lemurclaw_hooks::list_hooks(lemurclaw_hooks::HooksConfig {
...
"#,
    ...
    let test = test_codex()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = lemurclaw_config::types::ApprovalsReviewer::AutoReview;
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<lemurclaw_config::HooksFile>(plugin_hooks_json)
