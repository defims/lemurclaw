...
async fn build_harness_inner(
    ...
    TestCodexHarness::with_builder(builder.with_config(move |config| {
        config.cwd = lemurclaw_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
async fn capture_from_requests(
    mode: Mode,
    codex: &lemurclaw_core::CodexThread,
    rollout_path: &Path,
...
async fn submit_user_input(codex: &lemurclaw_core::CodexThread, items: Vec<UserInput>) -> Result<()> {
    codex
        .submit(Op::UserInput {
            items,
