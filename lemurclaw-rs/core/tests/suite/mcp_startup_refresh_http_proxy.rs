...
const PROXY_TEST_SUBPROCESS_ENV_VAR: &str = "LEMURCLAW_MCP_HTTP_PROXY_TEST_SUBPROCESS";
...
async fn local_mcp_startup_and_refresh_use_configured_http_client() -> Result<()> {
    ...
    if std::env::var_os(PROXY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        ...
        command
            ...
            .env("http_proxy", proxy.uri())
            .env("NO_PROXY", lemurclaw_network_proxy::DEFAULT_NO_PROXY_VALUE)
            .env("no_proxy", lemurclaw_network_proxy::DEFAULT_NO_PROXY_VALUE);
...
async fn skill_mcp_dependency_oauth_uses_configured_http_client() -> Result<()> {
    ...
    let servers = lemurclaw_config::load_global_mcp_servers(&fixture.config.codex_home).await?;
