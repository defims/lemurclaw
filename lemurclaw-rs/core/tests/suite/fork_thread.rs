...
use lemurclaw_core::parse_turn_item;
...
async fn assert_copied_fork_persists_inherited_history(history_mode: ThreadHistoryMode) {
    ...
    let source_meta = lemurclaw_rollout::read_session_meta_line(source_path.as_path())
        .await
    ...
    if matches!(history_mode, ThreadHistoryMode::Paginated) {
        ...
        let resumed_history = lemurclaw_rollout::RolloutRecorder::get_rollout_history(&forked_path)
            .await
        ...
        let resumed = thread_manager
            .resume_thread_with_history(
                test.config.clone(),
                resumed_history,
                lemurclaw_core::test_support::auth_manager_from_auth(
                    lemurclaw_login::CodexAuth::from_api_key("dummy"),
                ),
                /*parent_trace*/ None,
                /*supports_openai_form_elicitation*/ false,
            )
