...
use lemurclaw_core::config::ThreadStoreConfig;
...
use lemurclaw_protocol::protocol::ExecPolicyAmendment;
...
def main():
    args = parse_args()
    profile_name = os.environ.get("LEMURCLAW_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("LEMURCLAW_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("LEMURCLAW_PERMISSION_PROFILE is not allowlisted")
