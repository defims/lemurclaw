use lemurclaw_config::types::Personality;
...
use lemurclaw_protocol::openai_models::ModelInstructionsVariables;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = lemurclaw_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex()
        .with_auth(lemurclaw_login::CodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
