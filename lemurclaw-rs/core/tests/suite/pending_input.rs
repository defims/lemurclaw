...
use lemurclaw_extension_items::sleep::SleepItem;
...
impl lemurclaw_extension_api::ThreadLifecycleContributor<lemurclaw_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: lemurclaw_extension_api::ThreadStartInput<'a, lemurclaw_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions =
        lemurclaw_extension_api::ExtensionRegistryBuilder::<lemurclaw_core::config::Config>::new();
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            item,
            RolloutItem::ResponseItem(lemurclaw_protocol::models::ResponseItem::AgentMessage {
                content,
                ..
            }) if content.iter().any(|content| matches!(
                content,
                lemurclaw_protocol::models::AgentMessageInputContent::InputText { text }
