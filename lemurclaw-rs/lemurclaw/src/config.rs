...
//! `lemurclaw_tui::run_main` for the `tui` frontend; `gui` and `webui` are stubs.
...
pub enum Frontend {
    /// Terminal UI: pass through to `lemurclaw_tui::run_main`.
    #[default]
    #[value(name = "tui")]
    Tui,
    /// Native GUI: opens a wry+tao desktop window over the `lemurclaw://`
    /// protocol, embedding the shared React frontend.
    #[value(name = "gui")]
    Gui,
...
/// directly by `lemurclaw_tui::Cli` inside the TUI path; the fields here are the
...
/// full `lemurclaw_tui::Cli` from argv so it picks up every flag codex supports.
