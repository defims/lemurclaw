...
//! runs it. The `tui` frontend passes straight through to `lemurclaw_tui::run_main`
//! via `lemurclaw_arg0::arg0_dispatch_or_else`, mirroring `codex-rs/tui/src/main.rs`.
...
use lemurclaw_arg0::arg0_dispatch_or_else;
use lemurclaw_config::LoaderOverrides;
use lemurclaw_tui::AppExitInfo;
use lemurclaw_tui::Cli as TuiCli;
use lemurclaw_tui::ExitReason;
use lemurclaw_tui::run_main;
...
/// TUI frontend: pass through to `lemurclaw_tui::run_main` under
/// `arg0_dispatch_or_else` (which owns process exit on fatal errors).
fn run_tui() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|arg0_paths: Arg0DispatchPaths| async move {
        // Parse lemurclaw_tui::Cli from argv, but first strip lemurclaw-only flags
        // (--frontend / --agent-name) so lemurclaw_tui::Cli doesn't reject them as
...
/// control to `lemurclaw_tui::Cli` (which would reject them as unknown).
...
/// untouched so lemurclaw_tui::Cli sees exactly its own flags.
...
mod tests {
    ...
    fn strip_removes_webui_host_and_port() {
        // --host/--port are lemurclaw-owned (webui only); lemurclaw_tui::Cli must
