...
pub use lemurclaw_state::LogEntry;
use lemurclaw_state::SqliteConfig;
use lemurclaw_state::ThreadMetadataBuilder;
use lemurclaw_utils_path::normalize_for_path_comparison;
...
pub type StateDbHandle = Arc<lemurclaw_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        lemurclaw_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    lemurclaw_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == lemurclaw_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        lemurclaw_state::record_fallback(
            "get_state_db",
            "db_unavailable",
    ...
    let runtime = match lemurclaw_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> lemurclaw_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == lemurclaw_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<lemurclaw_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&lemurclaw_state::StateRuntime>,
    sqlite: &lemurclaw_state::SqliteConfig,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => lemurclaw_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => lemurclaw_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => lemurclaw_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<lemurclaw_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<lemurclaw_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    let state_sort_direction = match sort_direction {
        SortDirection::Asc => lemurclaw_state::SortDirection::Asc,
        SortDirection::Desc => lemurclaw_state::SortDirection::Desc,
    };

    if let Some(relation_filter) = relation_filter {
        let filters = lemurclaw_state::ThreadFilterOptions {
    ...
    Some(lemurclaw_state::ThreadsPage {
        items,
