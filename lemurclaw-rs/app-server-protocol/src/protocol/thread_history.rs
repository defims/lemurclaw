...
use lemurclaw_extension_items::image_generation::ImageGenerationItem;
use lemurclaw_protocol::items::parse_hook_prompt_message;
...
use lemurclaw_protocol::protocol::ContextCompactedEvent;
...
use lemurclaw_protocol::protocol::ExecCommandBeginEvent;
use lemurclaw_protocol::protocol::ExecCommandEndEvent;
use lemurclaw_protocol::protocol::GuardianAssessmentEvent;
use lemurclaw_protocol::protocol::GuardianAssessmentStatus;
use lemurclaw_protocol::protocol::ImageGenerationBeginEvent;
use lemurclaw_protocol::protocol::ImageGenerationEndEvent;
use lemurclaw_protocol::protocol::ItemCompletedEvent;
use lemurclaw_protocol::protocol::ItemStartedEvent;
use lemurclaw_protocol::protocol::McpToolCallBeginEvent;
use lemurclaw_protocol::protocol::McpToolCallEndEvent;
use lemurclaw_protocol::protocol::PatchApplyBeginEvent;
use lemurclaw_protocol::protocol::PatchApplyEndEvent;
...
use lemurclaw_protocol::protocol::ViewImageToolCallEvent;
use lemurclaw_protocol::protocol::WebSearchBeginEvent;
use lemurclaw_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use lemurclaw_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use lemurclaw_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use lemurclaw_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    fn handle_response_item(&mut self, item: &lemurclaw_protocol::models::ResponseItem) {
        let lemurclaw_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &lemurclaw_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            lemurclaw_protocol::items::TurnItem::EnteredReviewMode(_)
                | lemurclaw_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            lemurclaw_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            lemurclaw_protocol::items::TurnItem::HookPrompt(_)
            | lemurclaw_protocol::items::TurnItem::CommandExecution(_)
            | lemurclaw_protocol::items::TurnItem::DynamicToolCall(_)
            | lemurclaw_protocol::items::TurnItem::CollabAgentToolCall(_)
            | lemurclaw_protocol::items::TurnItem::SubAgentActivity(_)
            | lemurclaw_protocol::items::TurnItem::Extension(_)
            | lemurclaw_protocol::items::TurnItem::EnteredReviewMode(_)
            | lemurclaw_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            lemurclaw_protocol::items::TurnItem::UserMessage(_)
            | lemurclaw_protocol::items::TurnItem::AgentMessage(_)
            | lemurclaw_protocol::items::TurnItem::Reasoning(_)
            | lemurclaw_protocol::items::TurnItem::WebSearch(_)
            | lemurclaw_protocol::items::TurnItem::ImageView(_)
            | lemurclaw_protocol::items::TurnItem::ImageGeneration(_)
            | lemurclaw_protocol::items::TurnItem::FileChange(_)
            | lemurclaw_protocol::items::TurnItem::McpToolCall(_)
            | lemurclaw_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &lemurclaw_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &lemurclaw_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &lemurclaw_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &lemurclaw_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &lemurclaw_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &lemurclaw_protocol::protocol::ExitedReviewModeEvent,
    ) {
...
fn convert_dynamic_tool_content_items(
    items: &[lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use lemurclaw_extension_items::ExtensionItem as CoreExtensionItem;
    use lemurclaw_extension_items::sleep::SleepItem as CoreSleepItem;
    use lemurclaw_protocol::ThreadId;
    use lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use lemurclaw_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use lemurclaw_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use lemurclaw_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use lemurclaw_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use lemurclaw_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use lemurclaw_protocol::items::TurnItem as CoreTurnItem;
    use lemurclaw_protocol::items::UserMessageItem as CoreUserMessageItem;
    use lemurclaw_protocol::items::build_hook_prompt_message;
    use lemurclaw_protocol::mcp::CallToolResult;
    use lemurclaw_protocol::models::ImageDetail;
    use lemurclaw_protocol::models::MessagePhase as CoreMessagePhase;
    use lemurclaw_protocol::models::WebSearchAction as CoreWebSearchAction;
    use lemurclaw_protocol::parse_command::ParsedCommand;
    ...
    use lemurclaw_protocol::protocol::ExecCommandEndEvent;
    use lemurclaw_protocol::protocol::ExecCommandSource;
    use lemurclaw_protocol::protocol::ExitedReviewModeEvent;
    use lemurclaw_protocol::protocol::ItemStartedEvent;
    use lemurclaw_protocol::protocol::McpInvocation;
    use lemurclaw_protocol::protocol::McpToolCallEndEvent;
    use lemurclaw_protocol::protocol::PatchApplyBeginEvent;
    use lemurclaw_protocol::protocol::ReviewTarget;
    use lemurclaw_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![lemurclaw_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                lemurclaw_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    lemurclaw_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(lemurclaw_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(lemurclaw_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: lemurclaw_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(lemurclaw_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                lemurclaw_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                lemurclaw_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(lemurclaw_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(lemurclaw_protocol::models::ResponseItem::Message {
                id: Some(lemurclaw_protocol::ResponseItemId::with_suffix("msg", "1")),
                role: "user".into(),
                content: vec![lemurclaw_protocol::models::ContentItem::InputText {
