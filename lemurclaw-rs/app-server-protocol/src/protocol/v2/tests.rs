...
use lemurclaw_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use lemurclaw_protocol::config_types::MultiAgentMode;
use lemurclaw_protocol::items::AgentMessageContent;
use lemurclaw_protocol::items::AgentMessageItem;
use lemurclaw_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use lemurclaw_protocol::items::CollabAgentToolCallItem;
use lemurclaw_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use lemurclaw_protocol::items::CommandExecutionItem;
...
use lemurclaw_protocol::items::FileChangeItem;
use lemurclaw_protocol::items::ImageViewItem;
use lemurclaw_protocol::items::McpToolCallItem;
use lemurclaw_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use lemurclaw_protocol::items::ReasoningItem;
...
use lemurclaw_protocol::items::WebSearchItem as CoreWebSearchItem;
use lemurclaw_protocol::mcp::CallToolResult;
use lemurclaw_protocol::mcp::McpServerInfo;
use lemurclaw_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use lemurclaw_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use lemurclaw_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use lemurclaw_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use lemurclaw_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use lemurclaw_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use lemurclaw_protocol::protocol::AgentStatus as CoreAgentStatus;
use lemurclaw_protocol::protocol::AskForApproval as CoreAskForApproval;
use lemurclaw_protocol::protocol::ConversationTextRole;
use lemurclaw_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use lemurclaw_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use lemurclaw_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use lemurclaw_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use lemurclaw_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use lemurclaw_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: lemurclaw_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        lemurclaw_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        lemurclaw_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        lemurclaw_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![lemurclaw_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            lemurclaw_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = lemurclaw_protocol::ThreadId::default();
    let receiver_thread_id = lemurclaw_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: lemurclaw_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            lemurclaw_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(lemurclaw_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(lemurclaw_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(lemurclaw_protocol::config_types::CollaborationMode {
            mode: lemurclaw_protocol::config_types::ModeKind::Plan,
