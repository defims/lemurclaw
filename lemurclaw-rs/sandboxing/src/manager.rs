...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: lemurclaw_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing lemurclaw-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let codex_home = lemurclaw_utils_home_dir::find_codex_home()
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    codex_home: &Path,
    proxy_settings_mode: lemurclaw_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = lemurclaw_windows_sandbox::resolve_exe_for_launch(source.as_path(), codex_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        lemurclaw_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
