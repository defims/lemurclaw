...
//!
//! This crate wraps [`lemurclaw_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](lemurclaw_app_server::in_process::InProcessClientHandle),
...
pub use lemurclaw_app_server::app_server_control_socket_path;
pub use lemurclaw_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use lemurclaw_app_server::in_process::InProcessServerEvent;
use lemurclaw_app_server::in_process::InProcessStartArgs;
use lemurclaw_app_server::in_process::LogDbLayer;
pub use lemurclaw_app_server::in_process::StateDbHandle;
use lemurclaw_app_server_protocol::ClientInfo;
use lemurclaw_app_server_protocol::ClientNotification;
...
use lemurclaw_app_server_protocol::Result as JsonRpcResult;
...
use lemurclaw_config::NoopThreadConfigLoader;
use lemurclaw_config::RemoteThreadConfigLoader;
use lemurclaw_config::ThreadConfigLoader;
use lemurclaw_core::config::Config;
pub use lemurclaw_core::otel_init::build_provider as build_otel_provider;
...
pub mod legacy_core {
    pub mod config {
        pub use lemurclaw_core::config::*;

        pub mod edit {
            pub use lemurclaw_core::config::edit::*;
        }
    }
}
...
/// - [`lemurclaw_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            lemurclaw_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
mod tests {
    ...
    use lemurclaw_app_server_protocol::JSONRPCMessage;
    use lemurclaw_app_server_protocol::JSONRPCRequest;
    use lemurclaw_app_server_protocol::JSONRPCResponse;
    use lemurclaw_app_server_protocol::ServerNotification;
    use lemurclaw_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use lemurclaw_core::init_state_db;
    use lemurclaw_uds::UnixListener;
    use lemurclaw_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            lemurclaw_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            lemurclaw_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(lemurclaw_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: lemurclaw_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(lemurclaw_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: lemurclaw_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: lemurclaw_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: lemurclaw_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: lemurclaw_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<lemurclaw_app_server_protocol::ThreadReadResponse>(
            ...
            InProcessServerEvent::ServerNotification(notification)
                if matches!(
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            lemurclaw_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        )
                )
        ));
                ...
                if matches!(
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == lemurclaw_app_server_protocol::TurnStatus::Completed
                )
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: lemurclaw_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                lemurclaw_app_server_protocol::ServerNotification::TurnCompleted(
                    lemurclaw_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: lemurclaw_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: lemurclaw_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                lemurclaw_app_server_protocol::ServerNotification::AgentMessageDelta(
                    lemurclaw_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                lemurclaw_app_server_protocol::ServerNotification::ItemCompleted(
                    lemurclaw_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: lemurclaw_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                lemurclaw_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    lemurclaw_app_server_protocol::ExternalAgentConfigImportCompletedNotification {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                lemurclaw_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    lemurclaw_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            lemurclaw_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
