...
# Run a suite of smoke tests against the Windows sandbox via the lemurclaw CLI
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the lemurclaw CLI to invoke `lemurclaw sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run lemurclaw.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "lemurclaw.exe",
        ws_root / "target" / "release" / "lemurclaw.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "lemurclaw.exe",
            cargo_base / "release" / "lemurclaw.exe",
        ])
    ...
    if shutil.which("lemurclaw"):
        return ["lemurclaw"]

    raise FileNotFoundError(
        "lemurclaw CLI not found. Build it first, e.g.\n"
...
def run_sbx(
    ...
    # Map policy to lemurclaw CLI overrides.
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"LEMURCLAW_HOME": str(proxy_home)},
            )
    ...
    # 34. WS: policy tamper (.lemurclaw artifacts) denied
    codex_home = Path(os.environ["USERPROFILE"]) / ".lemurclaw"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .lemurclaw\\policy.json"], WS_ROOT)
    add("WS: .lemurclaw cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .lemurclaw policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://lemurclaw-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
