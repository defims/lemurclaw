...
`codex-app-server-daemon` backs the machine-readable `lemurclaw app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for lemurclaw instances launched over SSH, including fresh developer
...
```sh
lemurclaw app-server daemon start
lemurclaw app-server daemon restart
lemurclaw app-server daemon enable-remote-control
lemurclaw app-server daemon disable-remote-control
lemurclaw app-server daemon stop
lemurclaw app-server daemon version
lemurclaw app-server daemon bootstrap --remote-control
```
...
$HOME/.lemurclaw/packages/standalone/current/lemurclaw app-server daemon bootstrap --remote-control
```
...
The daemon assumes lemurclaw is installed through `install.sh` and always launches
the standalone managed binary under `CODEX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `CODEX_HOME/packages/standalone/current/lemurclaw` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `CODEX_HOME/packages/standalone/current/lemurclaw` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `lemurclaw remote-control` bootstraps with `--remote-control` when the
