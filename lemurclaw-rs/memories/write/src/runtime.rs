...
use lemurclaw_core::content_items_to_text;
use lemurclaw_core::detached_memory_responses_metadata;
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<lemurclaw_otel::Timer> {
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let lemurclaw_protocol::models::ResponseItem::Message { content, .. } = item
