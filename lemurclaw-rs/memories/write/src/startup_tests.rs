...
use lemurclaw_git_utils::diff_since_latest_init;
use lemurclaw_git_utils::reset_git_repository;
use lemurclaw_login::AuthManager;
use lemurclaw_login::CodexAuth;
use lemurclaw_model_provider::ModelProvider;
use lemurclaw_model_provider::ModelProviderFuture;
use lemurclaw_model_provider::ProviderAccountResult;
...
use lemurclaw_state::Phase2JobClaimOutcome;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<lemurclaw_state::StateRuntime>> {
    let db = lemurclaw_state::StateRuntime::init(
...
) -> (Arc<MemoryStartupContext>, Arc<lemurclaw_core::config::Config>) {
    let config_snapshot = test.codex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> lemurclaw_models_manager::manager::SharedModelsManager {
        self.delegate
...
async fn seed_stage1_output(
    db: &lemurclaw_state::StateRuntime,
    codex_home: &Path,
    ...
    let mut metadata_builder = lemurclaw_state::ThreadMetadataBuilder::new(
        thread_id,
...
) -> anyhow::Result<lemurclaw_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        lemurclaw_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
