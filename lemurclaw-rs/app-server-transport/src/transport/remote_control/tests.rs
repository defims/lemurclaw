...
use lemurclaw_app_server_protocol::ServerNotificationEnvelope;
use lemurclaw_config::types::AuthCredentialsStoreMode;
use lemurclaw_core::test_support::auth_manager_from_auth;
use lemurclaw_core::test_support::auth_manager_from_auth_with_home;
use lemurclaw_login::AuthDotJson;
use lemurclaw_login::AuthKeyringBackendKind;
use lemurclaw_login::AuthManager;
use lemurclaw_login::CodexAuth;
use lemurclaw_login::save_auth;
use lemurclaw_login::token_data::TokenData;
use lemurclaw_login::token_data::parse_chatgpt_jwt_claims;
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    lemurclaw_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(lemurclaw_app_server_protocol::JSONRPCRequest {
        id: lemurclaw_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(lemurclaw_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(lemurclaw_app_server_protocol::JSONRPCRequest {
                    id: lemurclaw_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(lemurclaw_app_server_protocol::JSONRPCRequest {
                id: lemurclaw_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
