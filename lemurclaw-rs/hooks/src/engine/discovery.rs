...
use lemurclaw_config::HookStateToml;
...
use lemurclaw_config::TomlValue;
use lemurclaw_config::version_for_toml;
use lemurclaw_plugin::PluginHookSource;
...
use lemurclaw_protocol::protocol::HookTrustStatus;
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: lemurclaw_protocol::protocol::HookEventName,
        ...
        for (handler_index, handler) in group.hooks.iter().cloned().enumerate() {
            match handler {
                HookHandlerConfig::Command {
                    ...
                    if r#async && event_name != lemurclaw_protocol::protocol::HookEventName::SessionEnd
                    {
                        warnings.push(format!(
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        lemurclaw_protocol::protocol::HookEventName::PreToolUse
                            | lemurclaw_protocol::protocol::HookEventName::PostToolUse
                            | lemurclaw_protocol::protocol::HookEventName::SessionStart
                            | lemurclaw_protocol::protocol::HookEventName::UserPromptSubmit
                            | lemurclaw_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != lemurclaw_protocol::protocol::HookEventName::SessionEnd {
