...
use lemurclaw_protocol::protocol::McpStartupFailure;
use lemurclaw_protocol::protocol::McpStartupStatus;
use lemurclaw_protocol::protocol::McpStartupUpdateEvent;
use lemurclaw_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            ...
            .map(lemurclaw_model_provider::auth_provider_from_auth);
        let codex_apps_auth_provider = codex_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_codex_backend()).map(|auth| {
                lemurclaw_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
            })
        });
