...
use lemurclaw_config::AppToolApproval;
use lemurclaw_config::Constrained;
use lemurclaw_config::McpServerConfig;
use lemurclaw_config::McpServerEnvVar;
use lemurclaw_config::McpServerToolConfig;
use lemurclaw_config::types::AuthKeyringBackendKind;
use lemurclaw_config::types::OAuthCredentialsStoreMode;
use lemurclaw_connectors::ConnectorRuntimeContext;
use lemurclaw_connectors::ConnectorRuntimeContextKey;
use lemurclaw_connectors::ConnectorRuntimeFetchSource;
use lemurclaw_connectors::ConnectorRuntimeManager;
use lemurclaw_exec_server_test_support::environment_manager_without_environments;
use lemurclaw_login::CodexAuth;
use lemurclaw_protocol::ToolName;
...
use lemurclaw_protocol::protocol::McpStartupFailureReason;
use lemurclaw_rmcp_client::ElicitationResponse;
use lemurclaw_rmcp_client::InProcessTransportFactory;
use lemurclaw_rmcp_client::McpAuthState;
use lemurclaw_rmcp_client::McpLoginRequirement;
use lemurclaw_rmcp_client::RmcpClient;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        lemurclaw_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        lemurclaw_protocol::mcp::RequestId::String(request_a_id),
        lemurclaw_protocol::mcp::RequestId::String(request_b_id),
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        lemurclaw_protocol::mcp::RequestId::String(cancelled_id),
        lemurclaw_protocol::mcp::RequestId::String(pending_id),
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<lemurclaw_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    mcp_config.protocol_mode = lemurclaw_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let connection_identity = |auth: &CodexAuth| {
        let provider = lemurclaw_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            OAuthCredentialsStoreMode::default(),
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = lemurclaw_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: lemurclaw_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = lemurclaw_login::test_support::transport_default_auth_route_config();
    let previous_auth = CodexAuth::AgentIdentity(
        lemurclaw_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        lemurclaw_login::auth::AgentIdentityAuthRecord {
