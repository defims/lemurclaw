...
use lemurclaw_app_server_client::ExecServerRuntimePaths;
...
use lemurclaw_app_server_protocol::Thread as AppServerThread;
use lemurclaw_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use lemurclaw_cloud_config::cloud_config_bundle_loader_for_storage;
use lemurclaw_config::CloudConfigBundleLoader;
use lemurclaw_config::ConfigLoadError;
use lemurclaw_config::ConfigLoadOptions;
use lemurclaw_config::LoaderOverrides;
use lemurclaw_config::format_config_error_with_source;
use lemurclaw_core::StateDbHandle;
use lemurclaw_core::check_execpolicy_for_warnings;
use lemurclaw_core::config::Config;
use lemurclaw_core::config::ConfigBuilder;
use lemurclaw_core::config::ConfigOverrides;
use lemurclaw_core::config::ConfigTomlLoadResult;
use lemurclaw_core::config::find_codex_home;
use lemurclaw_core::config::load_config_toml_with_layer_stack;
use lemurclaw_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use lemurclaw_core::config::resolve_bootstrap_auth_route_config;
use lemurclaw_core::config::resolve_oss_provider;
use lemurclaw_core::config::resolve_profile_v2_config_path;
use lemurclaw_core::find_thread_meta_by_name_str;
use lemurclaw_core::format_exec_policy_error_with_source;
use lemurclaw_core::path_utils;
use lemurclaw_feedback::CodexFeedback;
use lemurclaw_git_utils::get_git_repo_root;
use lemurclaw_login::AuthConfig;
use lemurclaw_login::default_client::set_default_client_residency_requirement;
use lemurclaw_login::default_client::set_default_originator;
use lemurclaw_login::enforce_login_restrictions;
use lemurclaw_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use lemurclaw_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use lemurclaw_otel::set_parent_from_context;
use lemurclaw_otel::traceparent_context_from_env;
...
use lemurclaw_protocol::protocol::RolloutLine;
...
use lemurclaw_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use lemurclaw_utils_cli::SharedCliOptions;
use lemurclaw_utils_oss::ensure_oss_provider_ready;
use lemurclaw_utils_oss::get_default_model_for_oss_provider;
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<lemurclaw_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        lemurclaw_core::otel_init::build_provider(
            &config,
    ...
    lemurclaw_core::otel_init::record_process_start(otel.as_ref(), "codex_exec");
    lemurclaw_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "codex_exec");
    ...
    let state_db = lemurclaw_core::init_state_db(&config).await;
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, lemurclaw_config::TomlValue)>,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = lemurclaw_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    if !json_mode
        && let Some(message) =
            lemurclaw_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        lemurclaw_app_server_protocol::TurnStatus::Failed
                            | lemurclaw_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<lemurclaw_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<lemurclaw_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<lemurclaw_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != lemurclaw_app_server_protocol::TurnItemsView::Full
}
