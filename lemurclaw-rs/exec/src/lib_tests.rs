...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = lemurclaw_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        lemurclaw_app_server_protocol::WarningNotification {
            thread_id: None,
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: lemurclaw_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            lemurclaw_app_server_protocol::Turn {
                id: "turn-1".to_string(),
    ...
    let notification =
        ServerNotification::TurnCompleted(lemurclaw_app_server_protocol::TurnCompletedNotification {
            ...
            turn: lemurclaw_app_server_protocol::Turn {
                id: "turn-1".to_string(),
                items_view: lemurclaw_app_server_protocol::TurnItemsView::Summary,
                items: Vec::new(),
                status: lemurclaw_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(lemurclaw_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(lemurclaw_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(lemurclaw_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(lemurclaw_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: lemurclaw_app_server_protocol::Thread {
            ...
            can_accept_direct_input: None,
            thread_source: Some(lemurclaw_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        approval_policy: lemurclaw_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: lemurclaw_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: lemurclaw_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
