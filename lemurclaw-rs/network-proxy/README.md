# lemurclaw-network-proxy

`lemurclaw-network-proxy` is lemurclaw's local network policy enforcement proxy. It runs:
...
`lemurclaw-network-proxy` reads from lemurclaw's merged `config.toml` (via `lemurclaw-core` config loading).
...
```bash
cargo run -p lemurclaw-network-proxy --
```
...
`lemurclaw-network-proxy` can be embedded as a library with a thin API:
...
When `lemurclaw-network-proxy` is embedded in managed lemurclaw runtime, policy decisions emit structured
OTEL-compatible events with `target=codex_otel.network_proxy`.

Event name:

- `lemurclaw.network_proxy.policy_decision`
...
This section documents the protections implemented by `lemurclaw-network-proxy`, and the boundaries of
