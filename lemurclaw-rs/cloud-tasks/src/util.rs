...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = lemurclaw_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    let ua = lemurclaw_login::default_client::get_codex_user_agent();
    ...
    {
        headers.extend(lemurclaw_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
