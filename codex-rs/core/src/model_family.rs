...
// code-rs additionally imports `ContextMode` here to power
...
fn apply_upstream_model_overrides(mut family: ModelFamily) -> ModelFamily {
    ...
    family.default_reasoning_effort = model_info
        ...
        .map(|effort| match effort {
            ...
            | codex_protocol::openai_models::ReasoningEffort::Max => ReasoningEffort::XHigh,
            // code-rs does not model `Ultra`; codex-rs does, so pass it through.
            codex_protocol::openai_models::ReasoningEffort::Ultra => ReasoningEffort::Ultra,
            codex_protocol::openai_models::ReasoningEffort::Custom(_) => ReasoningEffort::Medium,
        });
    ...
    family.supports_reasoning_summaries = model_info.supports_reasoning_summary_parameter;
    ...
    if let Some(tool_type) = model_info.apply_patch_tool_type.as_ref() {
        // codex-rs's ApplyPatchToolType only has the `Freeform` variant; code-rs
        // also has `Function`. We map both arms to `Freeform` here because the
        // upstream model_info already carries the resolved tool type, and the
        // codex-rs protocol enum simply doesn't model the `Function` case.
        family.apply_patch_tool_type = Some(match tool_type {
            codex_protocol::openai_models::ApplyPatchToolType::Freeform => {
    ...
    // code-rs's ModelInfo carries `prefer_websockets`; codex-rs's does not.
    // Default to false (HTTP transport) which matches codex-rs behaviour.
...
pub fn find_family_for_model(slug: &str) -> Option<ModelFamily> {
    ...
    if slug.starts_with("o3") {
        ...
        model_family!(slug, "gpt-oss", apply_patch_tool_type: Some(ApplyPatchToolType::Freeform),
            uses_local_shell_tool: true,
...
#[cfg(test)]
mod tests {
    // code-rs imports these from `crate::config_types` and
    // `crate::tool_apply_patch`; codex-rs has neither module, so we pull both
    // from `codex_protocol` instead (the protocol crate is the single source
    // of truth for these types in this fork).
    use codex_protocol::openai_models::ApplyPatchToolType;
    use codex_protocol::openai_models::ReasoningEffort;
    ...
    fn bundled_model_metadata_applies_upstream_tool_flags() {
        ...
        // Note: code-rs's ModelInfo carries `prefer_websockets`; codex-rs's does
        // not, so we cannot assert it here. It always defaults to false.
    }
...
// Note: code-rs additionally exposes `resolve_context_mode_limits` here, which
// consults a `ContextMode` enum (OneM / Auto / Disabled) to opt certain models
// into a 1M-token extended context window. codex-rs does not carry the
// `ContextMode` type, so this helper is intentionally omitted. If a future
// fork wants extended-context behaviour, port `ContextMode` into
// `codex_protocol::config_types` first and re-add this function using
// `EXTENDED_CONTEXT_WINDOW_1M` (still defined above) and
